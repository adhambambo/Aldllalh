...
class wrapping extends StatelessWidget {
  ...
  final Stream<QuerySnapshot> kochiFAcInfo = FirebaseFirestore.instance
      .collection('كولكشن/كولكشن/مواد تغليف')
              ...
              itemBuilder: (context, index) {
                ...
                if (factoryData.containsKey('banner')) {
                  return GestureDetector(
                    onTap: () {
                      Get.to(() =>
                          wrappiingbannersInfo(factoryDatatwo: factoryData));
                    },
                    child: Image.network(
                  ...
                  return GestureDetector(
                    onTap: () {
                      Get.to(() =>
                          wrappingcollecyioninfo(factoryData: factoryData));
                    },
...
//
class wrappingcollecyioninfo extends StatefulWidget {
  final Map<String, dynamic> factoryData;

  const wrappingcollecyioninfo({Key? key, required this.factoryData});

  @override
  State<wrappingcollecyioninfo> createState() => _wrappingcollecyioninfoState();
}

class _wrappingcollecyioninfoState extends State<wrappingcollecyioninfo> {
  final Stream<QuerySnapshot> kochiFAcInfo = FirebaseFirestore.instance
      .collection('كولكشن/كولكشن/مواد تغليف')
      .snapshots();
   int _current = 0;
...
class wrappiingbannersInfo extends StatefulWidget {
  final Map<String, dynamic> factoryDatatwo;

  wrappiingbannersInfo({Key? key, required this.factoryDatatwo})
      : super(key: key);

  @override
  State<wrappiingbannersInfo> createState() => _wrappiingbannersInfoState();
}

class _wrappiingbannersInfoState extends State<wrappiingbannersInfo> {
