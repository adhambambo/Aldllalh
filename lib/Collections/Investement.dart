...
class investement extends StatelessWidget {
  investement({super.key});
  final Stream<QuerySnapshot> kochiFAcInfo = FirebaseFirestore.instance
      .collection('كولكشن/كولكشن/استثمار عقاري')
              ...
              itemBuilder: (context, index) {
                ...
                if (factoryData.containsKey('banner')) {
                  return GestureDetector(
                    onTap: () {
                      Get.to(() =>
                          investemenytbannersInfo(factoryDatatwo: factoryData));
                    },
                    child: Image.network(
                  ...
                  return GestureDetector(
                    onTap: () {
                      Get.to(() =>
                          investementcollecyioninfo(factoryData: factoryData));
                    },
...
//
class investementcollecyioninfo extends StatefulWidget {
  final Map<String, dynamic> factoryData;

  const investementcollecyioninfo({Key? key, required this.factoryData});

  @override
  State<investementcollecyioninfo> createState() =>
      _investementcollecyioninfoState();
}

class _investementcollecyioninfoState extends State<investementcollecyioninfo> {
  final Stream<QuerySnapshot> kochiFAcInfo = FirebaseFirestore.instance
      .collection('كولكشن/كولكشن/استثمار عقاري')
      .snapshots();
...
class investemenytbannersInfo extends StatefulWidget {
  final Map<String, dynamic> factoryDatatwo;

  investemenytbannersInfo({Key? key, required this.factoryDatatwo})
      : super(key: key);

  @override
  State<investemenytbannersInfo> createState() =>
      _investemenytbannersInfoState();
}

class _investemenytbannersInfoState extends State<investemenytbannersInfo> {
