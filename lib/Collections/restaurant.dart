...
class Restaurant extends StatelessWidget {
  ...
  final Stream<QuerySnapshot> kochiFAcInfo = FirebaseFirestore.instance
      .collection('كولكشن/كولكشن/مطاعم')
              ...
              itemBuilder: (context, index) {
                ...
                if (factoryData.containsKey('banner')) {
                  return GestureDetector(
                    onTap: () {
                      Get.to(() =>
                          restautrenabannersInfo(factoryDatatwo: factoryData));
                    },
                    child: Image.network(
                  ...
                  return GestureDetector(
                    onTap: () {
                      Get.to(() =>
                          restaurantcollecyioninfo(factoryData: factoryData));
                    },
...
//
class restaurantcollecyioninfo extends StatefulWidget {
  final Map<String, dynamic> factoryData;

  const restaurantcollecyioninfo({Key? key, required this.factoryData});

  @override
  State<restaurantcollecyioninfo> createState() =>
      _restaurantcollecyioninfoState();
}

class _restaurantcollecyioninfoState extends State<restaurantcollecyioninfo> {
...
class restautrenabannersInfo extends StatefulWidget {
  final Map<String, dynamic> factoryDatatwo;

  restautrenabannersInfo({Key? key, required this.factoryDatatwo})
      : super(key: key);

  @override
  State<restautrenabannersInfo> createState() => _restautrenabannersInfoState();
}

class _restautrenabannersInfoState extends State<restautrenabannersInfo> {
  final Stream<QuerySnapshot> childreenFactories =
      FirebaseFirestore.instance.collection('كولكشن/كولكشن/مطاعم').snapshots();
