...
class cafe extends StatelessWidget {
  ...
  final Stream<QuerySnapshot> kochiFAcInfo = FirebaseFirestore.instance
      .collection('كولكشن/كولكشن/كافيهات')
              ...
              itemBuilder: (context, index) {
                ...
                if (factoryData.containsKey('banner')) {
                  return GestureDetector(
                    onTap: () {
                      Get.to(
                          () => cafebannersInfo(factoryDatatwo: factoryData));
                    },
                    child: Image.network(
                  ...
                  return GestureDetector(
                    onTap: () {
                      Get.to(
                          () => cafecollecyioninfo(factoryData: factoryData));
                    },
...
//
class cafecollecyioninfo extends StatefulWidget {
  final Map<String, dynamic> factoryData;

  const cafecollecyioninfo({Key? key, required this.factoryData});

  @override
  State<cafecollecyioninfo> createState() => _cafecollecyioninfoState();
}

class _cafecollecyioninfoState extends State<cafecollecyioninfo> {
  final Stream<QuerySnapshot> kochiFAcInfo = FirebaseFirestore.instance
      .collection('كولكشن/كولكشن/كافيهات')
      .snapshots();
...
class cafebannersInfo extends StatefulWidget {
  final Map<String, dynamic> factoryDatatwo;

  cafebannersInfo({Key? key, required this.factoryDatatwo}) : super(key: key);

  @override
  State<cafebannersInfo> createState() => _cafebannersInfoState();
}

class _cafebannersInfoState extends State<cafebannersInfo> {
