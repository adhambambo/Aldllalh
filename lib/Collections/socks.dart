...
class socks extends StatelessWidget {
  socks({super.key});
  final Stream<QuerySnapshot> kochiFAcInfo = FirebaseFirestore.instance
      .collection('كولكشن/كولكشن/شرابات')
              ...
              itemBuilder: (context, index) {
                ...
                if (factoryData.containsKey('banner')) {
                  return GestureDetector(
                    onTap: () {
                      Get.to(
                          () => socksbannersInfo(factoryDatatwo: factoryData));
                    },
                    child: Image.network(
                  ...
                  return GestureDetector(
                    onTap: () {
                      Get.to(
                          () => sockscollecyioninfo(factoryData: factoryData));
                    },
...
//
class sockscollecyioninfo extends StatefulWidget {
  final Map<String, dynamic> factoryData;

  const sockscollecyioninfo({Key? key, required this.factoryData});

  @override
  State<sockscollecyioninfo> createState() => _sockscollecyioninfoState();
}

class _sockscollecyioninfoState extends State<sockscollecyioninfo> {
...
class socksbannersInfo extends StatefulWidget {
  final Map<String, dynamic> factoryDatatwo;

  socksbannersInfo({Key? key, required this.factoryDatatwo}) : super(key: key);

  @override
  State<socksbannersInfo> createState() => _socksbannersInfoState();
}

class _socksbannersInfoState extends State<socksbannersInfo> {
  final Stream<QuerySnapshot> childreenFactories =
      FirebaseFirestore.instance.collection('كولكشن/كولكشن/شرابات').snapshots();
