...
class Printings extends StatelessWidget {
  Printings({super.key});
  final Stream<QuerySnapshot> kochiFAcInfo = FirebaseFirestore.instance
      .collection('كولكشن/كولكشن/طباعةوتطريز')
              ...
              itemBuilder: (context, index) {
                ...
                if (factoryData.containsKey('banner')) {
                  return GestureDetector(
                    onTap: () {
                      Get.to(() =>
                          printingssbannersInfo(factoryDatatwo: factoryData));
                    },
                    child: Image.network(
                  ...
                  return GestureDetector(
                    onTap: () {
                      Get.to(() =>
                          printingscollecyioninfo(factoryData: factoryData));
                    },
...
//
class printingscollecyioninfo extends StatefulWidget {
  final Map<String, dynamic> factoryData;

  const printingscollecyioninfo({Key? key, required this.factoryData});

  @override
  State<printingscollecyioninfo> createState() =>
      _printingscollecyioninfoState();
}

class _printingscollecyioninfoState extends State<printingscollecyioninfo> {
  final Stream<QuerySnapshot> kochiFAcInfo = FirebaseFirestore.instance
      .collection('كولكشن/كولكشن/طباعةوتطريز')
      .snapshots();
...
class printingssbannersInfo extends StatefulWidget {
  final Map<String, dynamic> factoryDatatwo;

  printingssbannersInfo({Key? key, required this.factoryDatatwo})
      : super(key: key);

  @override
  State<printingssbannersInfo> createState() => _printingssbannersInfoState();
}

class _printingssbannersInfoState extends State<printingssbannersInfo> {
