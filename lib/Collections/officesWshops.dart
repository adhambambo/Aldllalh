...
class officesShops extends StatelessWidget {
  officesShops({super.key});
  final Stream<QuerySnapshot> kochiFAcInfo = FirebaseFirestore.instance
      .collection('كولكشن/كولكشن/تجهيز محلات ومكاتب')
              ...
              itemBuilder: (context, index) {
                ...
                if (factoryData.containsKey('banner')) {
                  return GestureDetector(
                    onTap: () {
                      Get.to(() =>
                          officesshopsbannersInfo(factoryDatatwo: factoryData));
                    },
                    child: Image.network(
                  ...
                  return GestureDetector(
                    onTap: () {
                      Get.to(() =>
                          officesshopscollecyioninfo(factoryData: factoryData));
                    },
...
//
class officesshopscollecyioninfo extends StatefulWidget {
  final Map<String, dynamic> factoryData;

  const officesshopscollecyioninfo({Key? key, required this.factoryData});

  @override
  State<officesshopscollecyioninfo> createState() =>
      _officesshopscollecyioninfoState();
}

class _officesshopscollecyioninfoState
    extends State<officesshopscollecyioninfo> {
  final Stream<QuerySnapshot> kochiFAcInfo = FirebaseFirestore.instance
      .collection('كولكشن/كولكشن/تجهيز محلات ومكاتب')
      .snapshots();
...
class officesshopsbannersInfo extends StatefulWidget {
  final Map<String, dynamic> factoryDatatwo;

  officesshopsbannersInfo({Key? key, required this.factoryDatatwo})
      : super(key: key);

  @override
  State<officesshopsbannersInfo> createState() =>
      _officesshopsbannersInfoState();
}

class _officesshopsbannersInfoState extends State<officesshopsbannersInfo> {
