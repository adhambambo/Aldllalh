...
class HouseWare extends StatelessWidget {
  HouseWare({super.key});
  final Stream<QuerySnapshot> kochiFAcInfo = FirebaseFirestore.instance
      .collection('كولكشن/كولكشن/ادوات منزلية')
              ...
              itemBuilder: (context, index) {
                ...
                if (factoryData.containsKey('banner')) {
                  return GestureDetector(
                    onTap: () {
                      Get.to(() =>
                          housewarebannersInfo(factoryDatatwo: factoryData));
                    },
                    child: Image.network(
                  ...
                  return GestureDetector(
                    onTap: () {
                      Get.to(() =>
                          housewarecollectioninfo(factoryData: factoryData));
                    },
...
//
class housewarecollectioninfo extends StatefulWidget {
  final Map<String, dynamic> factoryData;

  const housewarecollectioninfo({Key? key, required this.factoryData});

  @override
  State<housewarecollectioninfo> createState() =>
      _housewarecollectioninfoState();
}

class _housewarecollectioninfoState extends State<housewarecollectioninfo> {
  final Stream<QuerySnapshot> kochiFAcInfo = FirebaseFirestore.instance
      .collection('كولكشن/كولكشن/ادوات منزلية')
      .snapshots();
...
class housewarebannersInfo extends StatefulWidget {
  final Map<String, dynamic> factoryDatatwo;

  housewarebannersInfo({Key? key, required this.factoryDatatwo})
      : super(key: key);

  @override
  State<housewarebannersInfo> createState() => _housewarebannersInfoState();
}

class _housewarebannersInfoState extends State<housewarebannersInfo> {
