...
class Apelliace extends StatelessWidget {
  Apelliace({super.key});
  final Stream<QuerySnapshot> kochiFAcInfo = FirebaseFirestore.instance
      .collection('كولكشن/كولكشن/اجهزة منزلية')
              ...
              itemBuilder: (context, index) {
                ...
                if (factoryData.containsKey('banner')) {
                  return GestureDetector(
                    onTap: () {
                      Get.to(() =>
                          appeliancebannersInfo(factoryDatatwo: factoryData));
                    },
                    child: Image.network(
                  ...
                  return GestureDetector(
                    onTap: () {
                      Get.to(() =>
                          apellaiancecollecyioninfo(factoryData: factoryData));
                    },
...
//
class apellaiancecollecyioninfo extends StatefulWidget {
  final Map<String, dynamic> factoryData;

  const apellaiancecollecyioninfo({Key? key, required this.factoryData});

  @override
  State<apellaiancecollecyioninfo> createState() =>
      _apellaiancecollecyioninfoState();
}

class _apellaiancecollecyioninfoState extends State<apellaiancecollecyioninfo> {
  final Stream<QuerySnapshot> kochiFAcInfo = FirebaseFirestore.instance
      .collection('كولكشن/كولكشن/اجهزة منزلية')
      .snapshots();
...
class appeliancebannersInfo extends StatefulWidget {
  final Map<String, dynamic> factoryDatatwo;

  appeliancebannersInfo({Key? key, required this.factoryDatatwo})
      : super(key: key);

  @override
  State<appeliancebannersInfo> createState() => _appeliancebannersInfoState();
}

class _appeliancebannersInfoState extends State<appeliancebannersInfo> {
