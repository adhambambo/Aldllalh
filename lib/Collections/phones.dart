...
class phones extends StatelessWidget {
  ...
  final Stream<QuerySnapshot> kochiFAcInfo = FirebaseFirestore.instance
      .collection('كولكشن/كولكشن/موبايلات')
              ...
              itemBuilder: (context, index) {
                ...
                if (factoryData.containsKey('banner')) {
                  return GestureDetector(
                    onTap: () {
                      Get.to(
                          () => phonesbannersInfo(factoryDatatwo: factoryData));
                    },
                    child: Image.network(
                  ...
                  return GestureDetector(
                    onTap: () {
                      Get.to(
                          () => phonescollectioninfo(factoryData: factoryData));
                    },
...
//
class phonescollectioninfo extends StatefulWidget {
  final Map<String, dynamic> factoryData;

  const phonescollectioninfo({Key? key, required this.factoryData});

  @override
  State<phonescollectioninfo> createState() => _phonescollectioninfoState();
}

class _phonescollectioninfoState extends State<phonescollectioninfo> {
  final Stream<QuerySnapshot> kochiFAcInfo = FirebaseFirestore.instance
      .collection('كولكشن/كولكشن/موبايلات')
      .snapshots();
...
class phonesbannersInfo extends StatefulWidget {
  final Map<String, dynamic> factoryDatatwo;

  phonesbannersInfo({Key? key, required this.factoryDatatwo}) : super(key: key);

  @override
  State<phonesbannersInfo> createState() => _phonesbannersInfoState();
}

class _phonesbannersInfoState extends State<phonesbannersInfo> {
