...
class beautyCentre extends StatelessWidget {
  beautyCentre({super.key});
  final Stream<QuerySnapshot> kochiFAcInfo = FirebaseFirestore.instance
      .collection('كولكشن/كولكشن/مراكز تجميل')
              ...
              itemBuilder: (context, index) {
                ...
                if (factoryData.containsKey('banner')) {
                  return GestureDetector(
                    onTap: () {
                      Get.to(() => beautycentersbannersInfo(
                  ...
                  return GestureDetector(
                    onTap: () {
                      Get.to(() =>
                          beautycentrecollecyioninfo(factoryData: factoryData));
                    },
...
//
class beautycentrecollecyioninfo extends StatefulWidget {
  final Map<String, dynamic> factoryData;

  const beautycentrecollecyioninfo({Key? key, required this.factoryData});

  @override
  State<beautycentrecollecyioninfo> createState() =>
      _beautycentrecollecyioninfoState();
}

class _beautycentrecollecyioninfoState
    extends State<beautycentrecollecyioninfo> {
  final Stream<QuerySnapshot> kochiFAcInfo = FirebaseFirestore.instance
      .collection('كولكشن/كولكشن/مراكز تجميل')
      .snapshots();
   int _current = 0;
...
class beautycentersbannersInfo extends StatefulWidget {
  final Map<String, dynamic> factoryDatatwo;

  beautycentersbannersInfo({Key? key, required this.factoryDatatwo})
      : super(key: key);

  @override
  State<beautycentersbannersInfo> createState() =>
      _beautycentersbannersInfoState();
}

class _beautycentersbannersInfoState extends State<beautycentersbannersInfo> {
