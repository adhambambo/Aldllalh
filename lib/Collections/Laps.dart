...
class laps extends StatelessWidget {
  ...
  final Stream<QuerySnapshot> kochiFAcInfo = FirebaseFirestore.instance
      .collection('كولكشن/كولكشن/معامل تحاليل')
              ...
              itemBuilder: (context, index) {
                ...
                if (factoryData.containsKey('banner')) {
                  return GestureDetector(
                    onTap: () {
                      Get.to(
                          () => lapsbannersInfo(factoryDatatwo: factoryData));
                    },
                    child: Image.network(
                  ...
                  return GestureDetector(
                    onTap: () {
                      Get.to(
                          () => lapscollecyioninfo(factoryData: factoryData));
                    },
...
//
class lapscollecyioninfo extends StatefulWidget {
  final Map<String, dynamic> factoryData;

  const lapscollecyioninfo({Key? key, required this.factoryData});

  @override
  State<lapscollecyioninfo> createState() => _lapscollecyioninfoState();
}

class _lapscollecyioninfoState extends State<lapscollecyioninfo> {
  final Stream<QuerySnapshot> kochiFAcInfo = FirebaseFirestore.instance
      .collection('كولكشن/كولكشن/معامل تحاليل')
      .snapshots();
   int _current = 0;
...
class lapsbannersInfo extends StatefulWidget {
  final Map<String, dynamic> factoryDatatwo;

  lapsbannersInfo({Key? key, required this.factoryDatatwo}) : super(key: key);

  @override
  State<lapsbannersInfo> createState() => _lapsbannersInfoState();
}

class _lapsbannersInfoState extends State<lapsbannersInfo> {
