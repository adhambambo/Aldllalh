...
class othersss extends StatelessWidget {
  othersss({super.key});
  final Stream<QuerySnapshot> kochiFAcInfo = FirebaseFirestore.instance
      .collection('كولكشن/كولكشن/اخري')
              ...
              itemBuilder: (context, index) {
                ...
                if (factoryData.containsKey('banner')) {
                  return GestureDetector(
                    onTap: () {
                      Get.to(() =>
                          otherssbannersInfo(factoryDatatwo: factoryData));
                    },
                    child: Image.network(
                  ...
                  return GestureDetector(
                    onTap: () {
                      Get.to(() =>
                          othserssscollecyioninfo(factoryData: factoryData));
                    },
...
//
class othserssscollecyioninfo extends StatefulWidget {
  final Map<String, dynamic> factoryData;

  const othserssscollecyioninfo({Key? key, required this.factoryData});

  @override
  State<othserssscollecyioninfo> createState() =>
      _othserssscollecyioninfoState();
}

class _othserssscollecyioninfoState extends State<othserssscollecyioninfo> {
...
class otherssbannersInfo extends StatefulWidget {
  final Map<String, dynamic> factoryDatatwo;

  otherssbannersInfo({Key? key, required this.factoryDatatwo})
      : super(key: key);

  @override
  State<otherssbannersInfo> createState() => _otherssbannersInfoState();
}

class _otherssbannersInfoState extends State<otherssbannersInfo> {
  final Stream<QuerySnapshot> childreenFactories =
      FirebaseFirestore.instance.collection('كولكشن/كولكشن/اخري').snapshots();
