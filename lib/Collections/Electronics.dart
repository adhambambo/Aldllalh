...
class Electroniccs extends StatelessWidget {
  Electroniccs({super.key});
  final Stream<QuerySnapshot> kochiFAcInfo = FirebaseFirestore.instance
      .collection('كولكشن/كولكشن/الكترونيات')
              ...
              itemBuilder: (context, index) {
                ...
                if (factoryData.containsKey('banner')) {
                  return GestureDetector(
                    onTap: () {
                      Get.to(() =>
                          electronicsbannersInfo(factoryDatatwo: factoryData));
                    },
                    child: Image.network(
                  ...
                  return GestureDetector(
                    onTap: () {
                      Get.to(() =>
                          electonicscollecyioninfo(factoryData: factoryData));
                    },
...
//
class electonicscollecyioninfo extends StatefulWidget {
  final Map<String, dynamic> factoryData;

  const electonicscollecyioninfo({Key? key, required this.factoryData});

  @override
  State<electonicscollecyioninfo> createState() =>
      _electonicscollecyioninfoState();
}

class _electonicscollecyioninfoState extends State<electonicscollecyioninfo> {
  final Stream<QuerySnapshot> kochiFAcInfo = FirebaseFirestore.instance
      .collection('كولكشن/كولكشن/الكترونيات')
      .snapshots();
...
class electronicsbannersInfo extends StatefulWidget {
  final Map<String, dynamic> factoryDatatwo;

  electronicsbannersInfo({Key? key, required this.factoryDatatwo})
      : super(key: key);

  @override
  State<electronicsbannersInfo> createState() => _electronicsbannersInfoState();
}

class _electronicsbannersInfoState extends State<electronicsbannersInfo> {
