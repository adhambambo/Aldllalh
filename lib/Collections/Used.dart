...
class used extends StatelessWidget {
  ...
  final Stream<QuerySnapshot> kochiFAcInfo = FirebaseFirestore.instance
      .collection('كولكشن/كولكشن/مستعمل')
              ...
              itemBuilder: (context, index) {
                ...
                if (factoryData.containsKey('banner')) {
                  return GestureDetector(
                    onTap: () {
                      Get.to(
                          () => usedbannersInfo(factoryDatatwo: factoryData));
                    },
                    child: Image.network(
                  ...
                  return GestureDetector(
                    onTap: () {
                      Get.to(
                          () => usedcollecyioninfo(factoryData: factoryData));
                    },
...
//
class usedcollecyioninfo extends StatefulWidget {
  final Map<String, dynamic> factoryData;

  const usedcollecyioninfo({Key? key, required this.factoryData});

  @override
  State<usedcollecyioninfo> createState() => _usedcollecyioninfoState();
}

class _usedcollecyioninfoState extends State<usedcollecyioninfo> {
...
class usedbannersInfo extends StatefulWidget {
  final Map<String, dynamic> factoryDatatwo;

  usedbannersInfo({Key? key, required this.factoryDatatwo}) : super(key: key);

  @override
  State<usedbannersInfo> createState() => _usedbannersInfoState();
}

class _usedbannersInfoState extends State<usedbannersInfo> {
  final Stream<QuerySnapshot> childreenFactories =
      FirebaseFirestore.instance.collection('كولكشن/كولكشن/مستعمل').snapshots();
