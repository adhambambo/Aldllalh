...
class Cosemetics extends StatelessWidget {
  Cosemetics({super.key});
  final Stream<QuerySnapshot> kochiFAcInfo = FirebaseFirestore.instance
      .collection('كولكشن/كولكشن/مستحضرات تجميل')
              ...
              itemBuilder: (context, index) {
                ...
                if (factoryData.containsKey('banner')) {
                  return GestureDetector(
                    onTap: () {
                      Get.to(() =>
                          cosemeticsbannersInfo(factoryDatatwo: factoryData));
                    },
                    child: Image.network(
                  ...
                  return GestureDetector(
                    onTap: () {
                      Get.to(() =>
                          cosemeticscollectioninfo(factoryData: factoryData));
                    },
...
//
class cosemeticscollectioninfo extends StatefulWidget {
  final Map<String, dynamic> factoryData;

  const cosemeticscollectioninfo({Key? key, required this.factoryData});

  @override
  State<cosemeticscollectioninfo> createState() =>
      _cosemeticscollectioninfoState();
}

class _cosemeticscollectioninfoState extends State<cosemeticscollectioninfo> {
  final Stream<QuerySnapshot> kochiFAcInfo = FirebaseFirestore.instance
      .collection('كولكشن/كولكشن/مستحضرات تجميل')
      .snapshots();
...
class cosemeticsbannersInfo extends StatefulWidget {
  final Map<String, dynamic> factoryDatatwo;

  cosemeticsbannersInfo({Key? key, required this.factoryDatatwo})
      : super(key: key);

  @override
  State<cosemeticsbannersInfo> createState() => _cosemeticsbannersInfoState();
}

class _cosemeticsbannersInfoState extends State<cosemeticsbannersInfo> {
