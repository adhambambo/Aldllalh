...
class chandeliear extends StatelessWidget {
  chandeliear({super.key});
  final Stream<QuerySnapshot> kochiFAcInfo = FirebaseFirestore.instance
      .collection('كولكشن/كولكشن/اضاءة و نجف')
              ...
              itemBuilder: (context, index) {
                ...
                if (factoryData.containsKey('banner')) {
                  return GestureDetector(
                    onTap: () {
                      Get.to(() =>
                          chandeliierbannersInfo(factoryDatatwo: factoryData));
                    },
                    child: Image.network(
                  ...
                  return GestureDetector(
                    onTap: () {
                      Get.to(() =>
                          chandeliercollecyioninfo(factoryData: factoryData));
                    },
...
//
class chandeliercollecyioninfo extends StatefulWidget {
  final Map<String, dynamic> factoryData;

  const chandeliercollecyioninfo({Key? key, required this.factoryData});

  @override
  State<chandeliercollecyioninfo> createState() =>
      _chandeliercollecyioninfoState();
}

class _chandeliercollecyioninfoState extends State<chandeliercollecyioninfo> {
  final Stream<QuerySnapshot> kochiFAcInfo = FirebaseFirestore.instance
      .collection('كولكشن/كولكشن/اضاءة و نجف')
      .snapshots();
...
class chandeliierbannersInfo extends StatefulWidget {
  final Map<String, dynamic> factoryDatatwo;

  chandeliierbannersInfo({Key? key, required this.factoryDatatwo})
      : super(key: key);

  @override
  State<chandeliierbannersInfo> createState() => _chandeliierbannersInfoState();
}

class _chandeliierbannersInfoState extends State<chandeliierbannersInfo> {
