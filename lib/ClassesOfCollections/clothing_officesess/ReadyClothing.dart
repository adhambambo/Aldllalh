...
class readyClothingOffices extends StatelessWidget {
  readyClothingOffices({super.key});
  final Stream<QuerySnapshot> kochiFAcInfo = FirebaseFirestore.instance
      .collection('مكاتب/مكاتب ملابس/مكاتب ملابس جاهزة')
              ...
              itemBuilder: (context, index) {
                ...
                if (factoryData.containsKey('banner')) {
                  return GestureDetector(
                    onTap: () {
                      Get.to(() =>
                          readyofficebannersInfo(factoryDatatwo: factoryData));
                    },
                    child: Image.network(
                  ...
                  return GestureDetector(
                    onTap: () {
                      Get.to(() =>
                          readyofficeProductioninfo(factoryData: factoryData));
                    },
...
//
class readyofficeProductioninfo extends StatefulWidget {
  final Map<String, dynamic> factoryData;

  const readyofficeProductioninfo({Key? key, required this.factoryData});

  @override
  State<readyofficeProductioninfo> createState() =>
      _readyofficeProductioninfoState();
}

class _readyofficeProductioninfoState extends State<readyofficeProductioninfo> {
  final Stream<QuerySnapshot> kochiFAcInfo = FirebaseFirestore.instance
      .collection('مكاتب/مكاتب ملابس/مكاتب ملابس جاهزة')
      .snapshots();
...
class readyofficebannersInfo extends StatefulWidget {
  final Map<String, dynamic> factoryDatatwo;

  readyofficebannersInfo({Key? key, required this.factoryDatatwo})
      : super(key: key);

  @override
  State<readyofficebannersInfo> createState() => _readyofficebannersInfoState();
}

class _readyofficebannersInfoState extends State<readyofficebannersInfo> {
