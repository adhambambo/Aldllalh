...
class AttabaMouskyClothingOffices extends StatelessWidget {
  AttabaMouskyClothingOffices({super.key});
  final Stream<QuerySnapshot> kochiFAcInfo = FirebaseFirestore.instance
      .collection('مكاتب/مكاتب ملابس/مكاتب ملابس العتبة والموسكي والغورية')
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        ...
        title: Text('مكاتب ملابس العتبة والموسكي والغورية'),
      ),
      body: Container(
        child: StreamBuilder(
          stream: kochiFAcInfo,
              ...
              itemBuilder: (context, index) {
                ...
                if (factoryData.containsKey('banner')) {
                  return GestureDetector(
                    onTap: () {
                      Get.to(() =>
                          attabaofficebannersInfo(factoryDatatwo: factoryData));
                    },
                    child: Image.network(
                  ...
                  return GestureDetector(
                    onTap: () {
                      Get.to(() =>
                          attabofficeProductioninfo(factoryData: factoryData));
                    },
...
//
class attabofficeProductioninfo extends StatefulWidget {
  final Map<String, dynamic> factoryData;

  const attabofficeProductioninfo({Key? key, required this.factoryData});

  @override
  State<attabofficeProductioninfo> createState() =>
      _attabofficeProductioninfoState();
}

class _attabofficeProductioninfoState extends State<attabofficeProductioninfo> {
  final Stream<QuerySnapshot> kochiFAcInfo = FirebaseFirestore.instance
      .collection('مكاتب/مكاتب ملابس/مكاتب ملابس العتبة والموسكي والغورية')
      .snapshots();
...
class attabaofficebannersInfo extends StatefulWidget {
  final Map<String, dynamic> factoryDatatwo;

  attabaofficebannersInfo({Key? key, required this.factoryDatatwo})
      : super(key: key);

  @override
  State<attabaofficebannersInfo> createState() =>
      _attabaofficebannersInfoState();
}

class _attabaofficebannersInfoState extends State<attabaofficebannersInfo> {
