...
class ShoazliaSednawiQudsClothingOffices extends StatelessWidget {
  ShoazliaSednawiQudsClothingOffices({super.key});
  final Stream<QuerySnapshot> kochiFAcInfo = FirebaseFirestore.instance
      .collection('مكاتب/مكاتب ملابس/مكاتب ملابس مول الشوازليه وصيدناوي والقدس')
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        ...
        title: Text('مكاتب ملابس الشوازليه وصيدناوي والقدس'),
      ),
      body: Container(
        child: StreamBuilder(
          stream: kochiFAcInfo,
              ...
              itemBuilder: (context, index) {
                ...
                if (factoryData.containsKey('banner')) {
                  return GestureDetector(
                    onTap: () {
                      Get.to(() => choislyofficebannersInfo(
                  ...
                  return GestureDetector(
                    onTap: () {
                      Get.to(() => choizlyofficeinfoProductioninfo(
...
//
class choizlyofficeinfoProductioninfo extends StatefulWidget {
  final Map<String, dynamic> factoryData;

  const choizlyofficeinfoProductioninfo({Key? key, required this.factoryData});

  @override
  State<choizlyofficeinfoProductioninfo> createState() =>
      _choizlyofficeinfoProductioninfoState();
}

class _choizlyofficeinfoProductioninfoState
    extends State<choizlyofficeinfoProductioninfo> {
  final Stream<QuerySnapshot> kochiFAcInfo = FirebaseFirestore.instance
      .collection('مكاتب/مكاتب ملابس/مكاتب ملابس مول الشوازليه وصيدناوي والقدس')
      .snapshots();
...
class choislyofficebannersInfo extends StatefulWidget {
  final Map<String, dynamic> factoryDatatwo;

  choislyofficebannersInfo({Key? key, required this.factoryDatatwo})
      : super(key: key);

  @override
  State<choislyofficebannersInfo> createState() =>
      _choislyofficebannersInfoState();
}

class _choislyofficebannersInfoState extends State<choislyofficebannersInfo> {
