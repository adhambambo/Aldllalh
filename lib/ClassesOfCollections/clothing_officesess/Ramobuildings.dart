...
class RamoBuildingClothingOffices extends StatelessWidget {
  RamoBuildingClothingOffices({super.key});
  final Stream<QuerySnapshot> kochiFAcInfo = FirebaseFirestore.instance
      .collection('مكاتب/مكاتب ملابس/مكاتب ملابس عمارات رامو')
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        ...
        title: Text('مكاتب ملابس عمارات رامو'),
      ),
      body: Container(
        child: StreamBuilder(
          stream: kochiFAcInfo,
              ...
              itemBuilder: (context, index) {
                ...
                if (factoryData.containsKey('banner')) {
                  return GestureDetector(
                    onTap: () {
                      Get.to(() =>
                          ramoofficebannersInfo(factoryDatatwo: factoryData));
                    },
                    child: Image.network(
                  ...
                  return GestureDetector(
                    onTap: () {
                      Get.to(() =>
                          ramobuildingofficeinfo(factoryData: factoryData));
                    },
...
//
class ramobuildingofficeinfo extends StatefulWidget {
  final Map<String, dynamic> factoryData;

  const ramobuildingofficeinfo({Key? key, required this.factoryData});

  @override
  State<ramobuildingofficeinfo> createState() => _ramobuildingofficeinfoState();
}

class _ramobuildingofficeinfoState extends State<ramobuildingofficeinfo> {
  final Stream<QuerySnapshot> kochiFAcInfo = FirebaseFirestore.instance
      .collection('مكاتب/مكاتب ملابس/مكاتب ملابس عمارات رامو')
      .snapshots();
...
class ramoofficebannersInfo extends StatefulWidget {
  final Map<String, dynamic> factoryDatatwo;

  ramoofficebannersInfo({Key? key, required this.factoryDatatwo})
      : super(key: key);

  @override
  State<ramoofficebannersInfo> createState() => _ramoofficebannersInfoState();
}

class _ramoofficebannersInfoState extends State<ramoofficebannersInfo> {
