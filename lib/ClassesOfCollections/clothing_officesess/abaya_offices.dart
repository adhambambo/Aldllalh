...
class AbayaEsdalatClothingOffices extends StatelessWidget {
  AbayaEsdalatClothingOffices({super.key});
  final Stream<QuerySnapshot> kochiFAcInfo = FirebaseFirestore.instance
      .collection('مكاتب/مكاتب ملابس/مكاتب ملابس عبايات واسدالات')
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        ...
        title: Text('مكاتب ملابس عبايات واسدالات'),
      ),
      body: Container(
        child: StreamBuilder(
          stream: kochiFAcInfo,
              ...
              itemBuilder: (context, index) {
                ...
                if (factoryData.containsKey('banner')) {
                  return GestureDetector(
                    onTap: () {
                      Get.to(() =>
                          abayaofficebannersInfo(factoryDatatwo: factoryData));
                    },
                    child: Image.network(
                  ...
                  return GestureDetector(
                    onTap: () {
                      Get.to(() =>
                          abayaofficeProductioninfo(factoryData: factoryData));
                    },
...
//
class abayaofficeProductioninfo extends StatefulWidget {
  final Map<String, dynamic> factoryData;

  const abayaofficeProductioninfo({Key? key, required this.factoryData});

  @override
  State<abayaofficeProductioninfo> createState() =>
      _abayaofficeProductioninfoState();
}

class _abayaofficeProductioninfoState extends State<abayaofficeProductioninfo> {
  final Stream<QuerySnapshot> kochiFAcInfo = FirebaseFirestore.instance
      .collection('مكاتب/مكاتب ملابس/مكاتب ملابس عبايات واسدالات')
      .snapshots();
...
class abayaofficebannersInfo extends StatefulWidget {
  final Map<String, dynamic> factoryDatatwo;

  abayaofficebannersInfo({Key? key, required this.factoryDatatwo})
      : super(key: key);

  @override
  State<abayaofficebannersInfo> createState() => _abayaofficebannersInfoState();
}

class _abayaofficebannersInfoState extends State<abayaofficebannersInfo> {
