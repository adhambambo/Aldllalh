...
class JeansClothingOffices extends StatelessWidget {
  JeansClothingOffices({super.key});
  final Stream<QuerySnapshot> kochiFAcInfo = FirebaseFirestore.instance
      .collection('مكاتب/مكاتب ملابس/مكاتب جينز')
              ...
              itemBuilder: (context, index) {
                ...
                if (factoryData.containsKey('banner')) {
                  return GestureDetector(
                    onTap: () {
                      Get.to(() =>
                          jeansofficebannersInfo(factoryDatatwo: factoryData));
                    },
                    child: Image.network(
                  ...
                  return GestureDetector(
                    onTap: () {
                      Get.to(() =>
                          jeansofficeProductioninfo(factoryData: factoryData));
                    },
...
//
class jeansofficeProductioninfo extends StatefulWidget {
  final Map<String, dynamic> factoryData;

  const jeansofficeProductioninfo({Key? key, required this.factoryData});

  @override
  State<jeansofficeProductioninfo> createState() =>
      _jeansofficeProductioninfoState();
}

class _jeansofficeProductioninfoState extends State<jeansofficeProductioninfo> {
  final Stream<QuerySnapshot> kochiFAcInfo = FirebaseFirestore.instance
      .collection('مكاتب/مكاتب ملابس/مكاتب جينز')
      .snapshots();
   int _current = 0;
...
class jeansofficebannersInfo extends StatefulWidget {
  final Map<String, dynamic> factoryDatatwo;

  jeansofficebannersInfo({Key? key, required this.factoryDatatwo})
      : super(key: key);

  @override
  State<jeansofficebannersInfo> createState() => _jeansofficebannersInfoState();
}

class _jeansofficebannersInfoState extends State<jeansofficebannersInfo> {
