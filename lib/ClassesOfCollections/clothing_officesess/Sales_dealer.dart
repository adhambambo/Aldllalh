...
class SalesDellares extends StatelessWidget {
  SalesDellares({super.key});
  final Stream<QuerySnapshot> kochiFAcInfo = FirebaseFirestore.instance
      .collection('مكاتب/مكاتب ملابس/تجار بيعات')
              ...
              itemBuilder: (context, index) {
                ...
                if (factoryData.containsKey('banner')) {
                  return GestureDetector(
                    onTap: () {
                      Get.to(() => SalesDellaresbannersInfo(
                  ...
                  return GestureDetector(
                    onTap: () {
                      Get.to(() =>
                          SalesDellaresinfo(factoryData: factoryData));
                    },
...
//
class SalesDellaresinfo extends StatefulWidget {
  final Map<String, dynamic> factoryData;

  const SalesDellaresinfo({Key? key, required this.factoryData});

  @override
  State<SalesDellaresinfo> createState() => _SalesDellaresinfoState();
}

class _SalesDellaresinfoState extends State<SalesDellaresinfo> {
  final Stream<QuerySnapshot> kochiFAcInfo = FirebaseFirestore.instance
      .collection('مكاتب/مكاتب ملابس/تجار بيعات')
      .snapshots();
...
class SalesDellaresbannersInfo extends StatefulWidget {
  final Map<String, dynamic> factoryDatatwo;

  SalesDellaresbannersInfo({Key? key, required this.factoryDatatwo})
      : super(key: key);

  @override
  State<SalesDellaresbannersInfo> createState() =>
      _SalesDellaresbannersInfoState();
}

class _SalesDellaresbannersInfoState
    extends State<SalesDellaresbannersInfo> {
