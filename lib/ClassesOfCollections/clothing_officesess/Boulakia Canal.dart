...
class BoulaqCanalClothingOffices extends StatelessWidget {
  BoulaqCanalClothingOffices({super.key});
  final Stream<QuerySnapshot> kochiFAcInfo = FirebaseFirestore.instance
      .collection('مكاتب/مكاتب ملابس/مكاتب ملابس الترعة البولاقية')
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        ...
        title: Text('مكاتب ملابس الترعه البولاقية'),
      ),
      body: Container(
        child: StreamBuilder(
          stream: kochiFAcInfo,
              ...
              itemBuilder: (context, index) {
                ...
                if (factoryData.containsKey('banner')) {
                  return GestureDetector(
                    onTap: () {
                      Get.to(() => boulakiapfficebannersInfo(
                  ...
                  return GestureDetector(
                    onTap: () {
                      Get.to(() => boulaqiacanalofficeProductioninfo(
...
//
class boulaqiacanalofficeProductioninfo extends StatefulWidget {
  final Map<String, dynamic> factoryData;

  const boulaqiacanalofficeProductioninfo(
      {Key? key, required this.factoryData});

  @override
  State<boulaqiacanalofficeProductioninfo> createState() =>
      _boulaqiacanalofficeProductioninfoState();
}

class _boulaqiacanalofficeProductioninfoState
    extends State<boulaqiacanalofficeProductioninfo> {
  final Stream<QuerySnapshot> kochiFAcInfo = FirebaseFirestore.instance
      .collection('مكاتب/مكاتب ملابس/مكاتب ملابس الترعة البولاقية')
      .snapshots();
        late int viewsCount;
...
class boulakiapfficebannersInfo extends StatefulWidget {
  final Map<String, dynamic> factoryDatatwo;

  boulakiapfficebannersInfo({Key? key, required this.factoryDatatwo})
      : super(key: key);

  @override
  State<boulakiapfficebannersInfo> createState() =>
      _boulakiapfficebannersInfoState();
}

class _boulakiapfficebannersInfoState extends State<boulakiapfficebannersInfo> {
