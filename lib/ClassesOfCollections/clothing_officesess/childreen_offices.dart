...
class ChildreenClothingOffices extends StatelessWidget {
  ChildreenClothingOffices({super.key});
  final Stream<QuerySnapshot> kochiFAcInfo = FirebaseFirestore.instance
      .collection('مكاتب/مكاتب ملابس/مكاتب ملابس اطفال')
              ...
              itemBuilder: (context, index) {
                ...
                if (factoryData.containsKey('banner')) {
                  return GestureDetector(
                    onTap: () {
                      Get.to(() => ChildreenofficebannersInfo(
                  ...
                  return GestureDetector(
                    onTap: () {
                      Get.to(() => childreenofficeinfoProductioninfo(
...
class childreenofficeinfoProductioninfo extends StatefulWidget {
  final Map<String, dynamic> factoryData;

  const childreenofficeinfoProductioninfo(
      {Key? key, required this.factoryData});

  @override
  State<childreenofficeinfoProductioninfo> createState() =>
      _childreenofficeinfoProductioninfoState();
}

// ignore: camel_case_types
class _childreenofficeinfoProductioninfoState
    extends State<childreenofficeinfoProductioninfo> {
  final Stream<QuerySnapshot> kochiFAcInfo = FirebaseFirestore.instance
      .collection('مكاتب/مكاتب ملابس/مكاتب ملابس اطفال')
      .snapshots();
...
class ChildreenofficebannersInfo extends StatefulWidget {
  final Map<String, dynamic> factoryDatatwo;

  ChildreenofficebannersInfo({Key? key, required this.factoryDatatwo})
      : super(key: key);

  @override
  State<ChildreenofficebannersInfo> createState() =>
      _ChildreenofficebannersInfoState();
}

class _ChildreenofficebannersInfoState
    extends State<ChildreenofficebannersInfo> {
