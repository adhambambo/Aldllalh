...
class LinjeriyClothingOffices extends StatelessWidget {
  LinjeriyClothingOffices({super.key});
  final Stream<QuerySnapshot> kochiFAcInfo = FirebaseFirestore.instance
      .collection('مكاتب/مكاتب ملابس/مكاتب داخلي ولانجيري')
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        ...
        title: Text('مكاتب داخلي ولانجيري'),
      ),
      body: Container(
        child: StreamBuilder(
          stream: kochiFAcInfo,
              ...
              itemBuilder: (context, index) {
                ...
                if (factoryData.containsKey('banner')) {
                  return GestureDetector(
                    onTap: () {
                      Get.to(() => linjerieofficebannersInfo(
                  ...
                  return GestureDetector(
                    onTap: () {
                      Get.to(() => linjerieyoffficeinfoProductioninfo(
...
//
class linjerieyoffficeinfoProductioninfo extends StatefulWidget {
  final Map<String, dynamic> factoryData;

  const linjerieyoffficeinfoProductioninfo(
      {Key? key, required this.factoryData});

  @override
  State<linjerieyoffficeinfoProductioninfo> createState() =>
      _linjerieyoffficeinfoProductioninfoState();
}

class _linjerieyoffficeinfoProductioninfoState
    extends State<linjerieyoffficeinfoProductioninfo> {
  final Stream<QuerySnapshot> kochiFAcInfo = FirebaseFirestore.instance
      .collection('مكاتب/مكاتب ملابس/مكاتب داخلي ولانجيري')
      .snapshots();
...
class linjerieofficebannersInfo extends StatefulWidget {
  final Map<String, dynamic> factoryDatatwo;

  linjerieofficebannersInfo({Key? key, required this.factoryDatatwo})
      : super(key: key);

  @override
  State<linjerieofficebannersInfo> createState() =>
      _linjerieofficebannersInfoState();
}

class _linjerieofficebannersInfoState extends State<linjerieofficebannersInfo> {
