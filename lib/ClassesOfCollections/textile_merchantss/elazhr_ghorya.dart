...
class ElazhrAndGhoryaTextileMerchants extends StatelessWidget {
  ElazhrAndGhoryaTextileMerchants({super.key});
  final Stream<QuerySnapshot> kochiFAcInfo = FirebaseFirestore.instance
      .collection('مكاتب/تجار اقمشة/اقمشة الازهر والغوريه')
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        ...
        title: Text('اقمشة الازهر والغوريه'),
      ),
      body: Container(
        child: StreamBuilder(
          stream: kochiFAcInfo,
              ...
              itemBuilder: (context, index) {
                ...
                if (factoryData.containsKey('banner')) {
                  return GestureDetector(
                    onTap: () {
                      Get.to(() =>
                          alazharbannersInfo(factoryDatatwo: factoryData));
                    },
                    child: Image.network(
                  ...
                  return GestureDetector(
                    onTap: () {
                      Get.to(() =>
                          azharghoryamerchantsinfo(factoryData: factoryData));
                    },
...
//
class azharghoryamerchantsinfo extends StatefulWidget {
  final Map<String, dynamic> factoryData;

  const azharghoryamerchantsinfo({Key? key, required this.factoryData});

  @override
  State<azharghoryamerchantsinfo> createState() =>
      _azharghoryamerchantsinfoState();
}

class _azharghoryamerchantsinfoState extends State<azharghoryamerchantsinfo> {
  final Stream<QuerySnapshot> kochiFAcInfo = FirebaseFirestore.instance
      .collection('مكاتب/تجار اقمشة/اقمشة الازهر والغوريه')
      .snapshots();
...
class alazharbannersInfo extends StatefulWidget {
  final Map<String, dynamic> factoryDatatwo;

  alazharbannersInfo({Key? key, required this.factoryDatatwo})
      : super(key: key);

  @override
  State<alazharbannersInfo> createState() => _alazharbannersInfoState();
}

class _alazharbannersInfoState extends State<alazharbannersInfo> {
