...
class GesrSuezTextileMerchants extends StatelessWidget {
  GesrSuezTextileMerchants({super.key});
  final Stream<QuerySnapshot> kochiFAcInfo = FirebaseFirestore.instance
      .collection('مكاتب/تجار اقمشة/اقمشة شارع 18 جسر السويس')
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        ...
        title: Text('اقمشة شارع 18 جسر السويس'),
      ),
      body: Container(
        child: StreamBuilder(
          stream: kochiFAcInfo,
              ...
              itemBuilder: (context, index) {
                ...
                if (factoryData.containsKey('banner')) {
                  return GestureDetector(
                    onTap: () {
                      Get.to(() => eighteeenstgesbannersInfo(
                  ...
                  return GestureDetector(
                    onTap: () {
                      Get.to(() => eightennstsuezmerchantsinfo(
...
//
class eightennstsuezmerchantsinfo extends StatefulWidget {
  final Map<String, dynamic> factoryData;

  const eightennstsuezmerchantsinfo({Key? key, required this.factoryData});

  @override
  State<eightennstsuezmerchantsinfo> createState() =>
      _eightennstsuezmerchantsinfoState();
}

class _eightennstsuezmerchantsinfoState
    extends State<eightennstsuezmerchantsinfo> {
  final Stream<QuerySnapshot> kochiFAcInfo = FirebaseFirestore.instance
      .collection('مكاتب/تجار اقمشة/اقمشة شارع 18 جسر السويس')
      .snapshots();
...
class eighteeenstgesbannersInfo extends StatefulWidget {
  final Map<String, dynamic> factoryDatatwo;

  eighteeenstgesbannersInfo({Key? key, required this.factoryDatatwo})
      : super(key: key);

  @override
  State<eighteeenstgesbannersInfo> createState() =>
      _eighteeenstgesbannersInfoState();
}

class _eighteeenstgesbannersInfoState extends State<eighteeenstgesbannersInfo> {
