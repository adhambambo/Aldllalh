...
class AlbalhTextileMerchants extends StatelessWidget {
  AlbalhTextileMerchants({super.key});
  final Stream<QuerySnapshot> kochiFAcInfo = FirebaseFirestore.instance
      .collection('مكاتب/تجار اقمشة/اقمشة وكالة البلح')
              ...
              itemBuilder: (context, index) {
                ...
                if (factoryData.containsKey('banner')) {
                  return GestureDetector(
                    onTap: () {
                      Get.to(() => AlballahagencybannersInfo(
                  ...
                  return GestureDetector(
                    onTap: () {
                      Get.to(() =>
                          albalhagencymerchantsinfo(factoryData: factoryData));
                    },
...
//
class albalhagencymerchantsinfo extends StatefulWidget {
  final Map<String, dynamic> factoryData;

  const albalhagencymerchantsinfo({Key? key, required this.factoryData});

  @override
  State<albalhagencymerchantsinfo> createState() =>
      _albalhagencymerchantsinfoState();
}

class _albalhagencymerchantsinfoState extends State<albalhagencymerchantsinfo> {
  final Stream<QuerySnapshot> kochiFAcInfo = FirebaseFirestore.instance
      .collection('مكاتب/تجار اقمشة/اقمشة وكالة البلح')
      .snapshots();
...
class AlballahagencybannersInfo extends StatefulWidget {
  final Map<String, dynamic> factoryDatatwo;

  AlballahagencybannersInfo({Key? key, required this.factoryDatatwo})
      : super(key: key);

  @override
  State<AlballahagencybannersInfo> createState() =>
      _AlballahagencybannersInfoState();
}

class _AlballahagencybannersInfoState extends State<AlballahagencybannersInfo> {
