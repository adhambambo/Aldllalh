...
class ElmatryaTextileMerchants extends StatelessWidget {
  ElmatryaTextileMerchants({super.key});
  final Stream<QuerySnapshot> kochiFAcInfo = FirebaseFirestore.instance
      .collection('مكاتب/تجار اقمشة/اقمشة المطرية')
              ...
              itemBuilder: (context, index) {
                ...
                if (factoryData.containsKey('banner')) {
                  return GestureDetector(
                    onTap: () {
                      Get.to(() =>
                          almatreyyabannersInfo(factoryDatatwo: factoryData));
                    },
                    child: Image.network(
                  ...
                  return GestureDetector(
                    onTap: () {
                      Get.to(
                          () => matryaMerchantsinfo(factoryData: factoryData));
                    },
...
//
class matryaMerchantsinfo extends StatefulWidget {
  final Map<String, dynamic> factoryData;

  const matryaMerchantsinfo({Key? key, required this.factoryData});

  @override
  State<matryaMerchantsinfo> createState() => _matryaMerchantsinfoState();
}

class _matryaMerchantsinfoState extends State<matryaMerchantsinfo> {
  final Stream<QuerySnapshot> kochiFAcInfo = FirebaseFirestore.instance
      .collection('مكاتب/تجار اقمشة/اقمشة المطرية')
      .snapshots();
...
class almatreyyabannersInfo extends StatefulWidget {
  final Map<String, dynamic> factoryDatatwo;

  almatreyyabannersInfo({Key? key, required this.factoryDatatwo})
      : super(key: key);

  @override
  State<almatreyyabannersInfo> createState() => _almatreyyabannersInfoState();
}

class _almatreyyabannersInfoState extends State<almatreyyabannersInfo> {
