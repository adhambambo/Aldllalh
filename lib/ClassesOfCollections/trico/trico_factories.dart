...
class tricoTrico extends StatelessWidget {
  tricoTrico({super.key});
  final Stream<QuerySnapshot> kochi = FirebaseFirestore.instance
      .collection('مصانع/تريكو/تريكو')
              ...
              itemBuilder: (context, index) {
                ...
                if (factoryData.containsKey('banner')) {
                  return GestureDetector(
                    onTap: () {
                      Get.to(() =>
                          tricotricobannersInfo(factoryDatatwo: factoryData));
                    },
                    child: Image.network(
                  ...
                  return GestureDetector(
                    onTap: () {
                      Get.to(() => tricoTricoFacInfo(factoryData: factoryData));
                    },
...
//
class tricoTricoFacInfo extends StatefulWidget {
  final Map<String, dynamic> factoryData;

  const tricoTricoFacInfo({Key? key, required this.factoryData});

  @override
  State<tricoTricoFacInfo> createState() => _tricoTricoFacInfoState();
}

class _tricoTricoFacInfoState extends State<tricoTricoFacInfo> {
  final Stream<QuerySnapshot> kochiFAcInfo =
...
class tricotricobannersInfo extends StatefulWidget {
  final Map<String, dynamic> factoryDatatwo;

  tricotricobannersInfo({Key? key, required this.factoryDatatwo})
      : super(key: key);

  @override
  State<tricotricobannersInfo> createState() => _tricotricobannersInfoState();
}

class _tricotricobannersInfoState extends State<tricotricobannersInfo> {
  final Stream<QuerySnapshot> childreenFactories =
      FirebaseFirestore.instance.collection('مصانع/تريكو/تريكو').snapshots();
