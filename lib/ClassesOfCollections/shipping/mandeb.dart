...
class manadeb extends StatelessWidget {
  manadeb({super.key});
  final Stream<QuerySnapshot> kochiFAcInfo = FirebaseFirestore.instance
      .collection('شركات شحن/شركات شحن/مناديب دفع مقدم')
              ...
              itemBuilder: (context, index) {
                ...
                if (factoryData.containsKey('banner')) {
                  return GestureDetector(
                    onTap: () {
                      Get.to(() =>
                          manadeebbannersInfo(factoryDatatwo: factoryData));
                    },
                    child: Image.network(
                  ...
                  return GestureDetector(
                    onTap: () {
                      Get.to(() => manadebinfo(factoryData: factoryData));
                    },
...
//
class manadebinfo extends StatefulWidget {
  final Map<String, dynamic> factoryData;

  const manadebinfo({Key? key, required this.factoryData});

  @override
  State<manadebinfo> createState() => _manadebinfoState();
}

class _manadebinfoState extends State<manadebinfo> {
  final Stream<QuerySnapshot> kochiFAcInfo = FirebaseFirestore.instance
      .collection('شركات شحن/شركات شحن/مناديب دفع مقدم')
      .snapshots();
...
class manadeebbannersInfo extends StatefulWidget {
  final Map<String, dynamic> factoryDatatwo;

  manadeebbannersInfo({Key? key, required this.factoryDatatwo})
      : super(key: key);

  @override
  State<manadeebbannersInfo> createState() => _manadeebbannersInfoState();
}

class _manadeebbannersInfoState extends State<manadeebbannersInfo> {
