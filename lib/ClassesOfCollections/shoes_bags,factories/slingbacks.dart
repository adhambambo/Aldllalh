...
class SlingbacksShoesBagsFactories extends StatelessWidget {
  SlingbacksShoesBagsFactories({super.key});
  final Stream<QuerySnapshot> slingbacks = FirebaseFirestore.instance
      .collection('مصانع/مصانع احذية وشنط/مصانع احذية خفيفه')
              ...
              itemBuilder: (context, index) {
                ...
                if (factoryData.containsKey('banner')) {
                  return GestureDetector(
                    onTap: () {
                      Get.to(() =>
                          slingbacksbannersInfo(factoryDatatwo: factoryData));
                    },
                    child: Image.network(
                  ...
                  return GestureDetector(
                    onTap: () {
                      Get.to(() => slingbackfacInfo(factoryData: factoryData));
                    },
...
//
class slingbackfacInfo extends StatefulWidget {
  final Map<String, dynamic> factoryData;

  const slingbackfacInfo({Key? key, required this.factoryData});

  @override
  State<slingbackfacInfo> createState() => _slingbackfacInfoState();
}

class _slingbackfacInfoState extends State<slingbackfacInfo> {
  final Stream<QuerySnapshot> slingbackfacInfo = FirebaseFirestore.instance
      .collection('مصانع/مصانع احذية وشنط/مصانع احذية خفيفه')
      .snapshots();
   int _current = 0;
     late int viewsCount;
...
class slingbacksbannersInfo extends StatefulWidget {
  final Map<String, dynamic> factoryDatatwo;

  slingbacksbannersInfo({Key? key, required this.factoryDatatwo})
      : super(key: key);

  @override
  State<slingbacksbannersInfo> createState() => _slingbacksbannersInfoState();
}

class _slingbacksbannersInfoState extends State<slingbacksbannersInfo> {
