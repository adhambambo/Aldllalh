...
class Linjerieyshops extends StatelessWidget {
  Linjerieyshops({super.key});
  final Stream<QuerySnapshot> kochiFAcInfo = FirebaseFirestore.instance
      .collection('محلات/محلات/محلات لانجيري')
              ...
              itemBuilder: (context, index) {
                ...
                if (factoryData.containsKey('banner')) {
                  return GestureDetector(
                    onTap: () {
                      Get.to(() =>
                          linjerieshopbannersInfo(factoryDatatwo: factoryData));
                    },
                    child: Image.network(
                  ...
                  return GestureDetector(
                    onTap: () {
                      Get.to(() => linjeriyshops(factoryData: factoryData));
                    },
...
//
class linjeriyshops extends StatefulWidget {
  final Map<String, dynamic> factoryData;

  const linjeriyshops({Key? key, required this.factoryData});

  @override
  State<linjeriyshops> createState() => _linjeriyshopsState();
}

class _linjeriyshopsState extends State<linjeriyshops> {
  final Stream<QuerySnapshot> kochiFAcInfo = FirebaseFirestore.instance
      .collection('محلات/محلات/محلات لانجيري')
      .snapshots();
...
class linjerieshopbannersInfo extends StatefulWidget {
  final Map<String, dynamic> factoryDatatwo;

  linjerieshopbannersInfo({Key? key, required this.factoryDatatwo})
      : super(key: key);

  @override
  State<linjerieshopbannersInfo> createState() =>
      _linjerieshopbannersInfoState();
}

class _linjerieshopbannersInfoState extends State<linjerieshopbannersInfo> {
