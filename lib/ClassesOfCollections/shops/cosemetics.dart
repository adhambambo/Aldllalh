...
class cosemeticsshops extends StatelessWidget {
  cosemeticsshops({super.key});
  final Stream<QuerySnapshot> kochiFAcInfo = FirebaseFirestore.instance
      .collection('محلات/محلات/محلات مستحضرات تجميل')
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        ...
        title: Text('محلات مستحضرات تجميل'),
      ),
      body: Container(
        child: StreamBuilder(
          stream: kochiFAcInfo,
              ...
              itemBuilder: (context, index) {
                ...
                if (factoryData.containsKey('banner')) {
                  return GestureDetector(
                    onTap: () {
                      Get.to(() => cosmeticsshopbannersInfo(
                  ...
                  return GestureDetector(
                    onTap: () {
                      Get.to(
                          () => cosmeticsShopsinfo(factoryData: factoryData));
                    },
...
//
class cosmeticsShopsinfo extends StatefulWidget {
  final Map<String, dynamic> factoryData;

  const cosmeticsShopsinfo({Key? key, required this.factoryData});

  @override
  State<cosmeticsShopsinfo> createState() => _cosmeticsShopsinfoState();
}

class _cosmeticsShopsinfoState extends State<cosmeticsShopsinfo> {
  final Stream<QuerySnapshot> kochiFAcInfo = FirebaseFirestore.instance
      .collection('محلات/محلات/محلات مستحضرات تجميل')
      .snapshots();
...
class cosmeticsshopbannersInfo extends StatefulWidget {
  final Map<String, dynamic> factoryDatatwo;

  cosmeticsshopbannersInfo({Key? key, required this.factoryDatatwo})
      : super(key: key);

  @override
  State<cosmeticsshopbannersInfo> createState() =>
      _cosmeticsshopbannersInfoState();
}

class _cosmeticsshopbannersInfoState extends State<cosmeticsshopbannersInfo> {
