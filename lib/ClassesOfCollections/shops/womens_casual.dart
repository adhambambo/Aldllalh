...
class WomensCasulaShops extends StatelessWidget {
  WomensCasulaShops({super.key});
  final Stream<QuerySnapshot> kochiFAcInfo = FirebaseFirestore.instance
      .collection('محلات/محلات/محلات حريمي كاجوال')
              ...
              itemBuilder: (context, index) {
                ...
                if (factoryData.containsKey('banner')) {
                  return GestureDetector(
                    onTap: () {
                      Get.to(() => womenscasualshopbannersInfo(
                  ...
                  return GestureDetector(
                    onTap: () {
                      Get.to(() =>
                          womenscasualshopsunfo(factoryData: factoryData));
                    },
...
//
class womenscasualshopsunfo extends StatefulWidget {
  final Map<String, dynamic> factoryData;

  const womenscasualshopsunfo({Key? key, required this.factoryData});

  @override
  State<womenscasualshopsunfo> createState() => _womenscasualshopsunfoState();
}

class _womenscasualshopsunfoState extends State<womenscasualshopsunfo> {
  final Stream<QuerySnapshot> kochiFAcInfo = FirebaseFirestore.instance
      .collection('محلات/محلات/محلات حريمي كاجوال')
      .snapshots();
...
class womenscasualshopbannersInfo extends StatefulWidget {
  final Map<String, dynamic> factoryDatatwo;

  womenscasualshopbannersInfo({Key? key, required this.factoryDatatwo})
      : super(key: key);

  @override
  State<womenscasualshopbannersInfo> createState() =>
      _womenscasualshopbannersInfoState();
}

class _womenscasualshopbannersInfoState
    extends State<womenscasualshopbannersInfo> {
