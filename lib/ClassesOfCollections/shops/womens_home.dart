...
class WomensHomeshops extends StatelessWidget {
  WomensHomeshops({super.key});
  final Stream<QuerySnapshot> kochiFAcInfo = FirebaseFirestore.instance
      .collection('محلات/محلات/محلات ملابس حريمي بيتي')
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        ...
        title: Text('محلات ملابس حريمي بيتي'),
      ),
      body: Container(
        child: StreamBuilder(
          stream: kochiFAcInfo,
              ...
              itemBuilder: (context, index) {
                ...
                if (factoryData.containsKey('banner')) {
                  return GestureDetector(
                    onTap: () {
                      Get.to(() =>
                          womenshomebannersInfo(factoryDatatwo: factoryData));
                    },
                    child: Image.network(
                  ...
                  return GestureDetector(
                    onTap: () {
                      Get.to(
                          () => womenshomeshopsinfo(factoryData: factoryData));
                    },
...
//
class womenshomeshopsinfo extends StatefulWidget {
  final Map<String, dynamic> factoryData;

  const womenshomeshopsinfo({Key? key, required this.factoryData});

  @override
  State<womenshomeshopsinfo> createState() => _womenshomeshopsinfoState();
}

class _womenshomeshopsinfoState extends State<womenshomeshopsinfo> {
  final Stream<QuerySnapshot> kochiFAcInfo = FirebaseFirestore.instance
      .collection('محلات/محلات/محلات ملابس حريمي بيتي')
      .snapshots();
...
class womenshomebannersInfo extends StatefulWidget {
  final Map<String, dynamic> factoryDatatwo;

  womenshomebannersInfo({Key? key, required this.factoryDatatwo})
      : super(key: key);

  @override
  State<womenshomebannersInfo> createState() => _womenshomebannersInfoState();
}

class _womenshomebannersInfoState extends State<womenshomebannersInfo> {
