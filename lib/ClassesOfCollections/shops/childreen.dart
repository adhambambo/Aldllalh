...
class Childreenshops extends StatelessWidget {
  Childreenshops({super.key});
  final Stream<QuerySnapshot> kochiFAcInfo = FirebaseFirestore.instance
      .collection('محلات/محلات/محلات ملابس اطفال')
              ...
              itemBuilder: (context, index) {
                ...
                if (factoryData.containsKey('banner')) {
                  return GestureDetector(
                    onTap: () {
                      Get.to(() => ChildreenshopbannersInfo(
                  ...
                  return GestureDetector(
                    onTap: () {
                      Get.to(
                          () => childreenshopsinfo(factoryData: factoryData));
                    },
...
//
class childreenshopsinfo extends StatefulWidget {
  final Map<String, dynamic> factoryData;

  const childreenshopsinfo({Key? key, required this.factoryData});

  @override
  State<childreenshopsinfo> createState() => _childreenshopsinfoState();
}

class _childreenshopsinfoState extends State<childreenshopsinfo> {
  final Stream<QuerySnapshot> kochiFAcInfo = FirebaseFirestore.instance
      .collection('محلات/محلات/محلات ملابس اطفال')
      .snapshots();
...
class ChildreenshopbannersInfo extends StatefulWidget {
  final Map<String, dynamic> factoryDatatwo;

  ChildreenshopbannersInfo({Key? key, required this.factoryDatatwo})
      : super(key: key);

  @override
  State<ChildreenshopbannersInfo> createState() =>
      _ChildreenshopbannersInfoState();
}

class _ChildreenshopbannersInfoState extends State<ChildreenshopbannersInfo> {
