...
class Othersshops extends StatelessWidget {
  Othersshops({super.key});
  final Stream<QuerySnapshot> kochiFAcInfo = FirebaseFirestore.instance
      .collection('محلات/محلات/محلات اخري')
              ...
              itemBuilder: (context, index) {
                ...
                if (factoryData.containsKey('banner')) {
                  return GestureDetector(
                    onTap: () {
                      Get.to(() =>
                          othersshopsbannersInfo(factoryDatatwo: factoryData));
                    },
                    child: Image.network(
                  ...
                  return GestureDetector(
                    onTap: () {
                      Get.to(() => othersshopsinfo(factoryData: factoryData));
                    },
...
//
class othersshopsinfo extends StatefulWidget {
  final Map<String, dynamic> factoryData;

  const othersshopsinfo({Key? key, required this.factoryData});

  @override
  State<othersshopsinfo> createState() => _othersshopsinfoState();
}

class _othersshopsinfoState extends State<othersshopsinfo> {
  final Stream<QuerySnapshot> kochiFAcInfo = FirebaseFirestore.instance
      .collection('محلات/محلات/محلات اخري')
      .snapshots();
...
class othersshopsbannersInfo extends StatefulWidget {
  final Map<String, dynamic> factoryDatatwo;

  othersshopsbannersInfo({Key? key, required this.factoryDatatwo})
      : super(key: key);

  @override
  State<othersshopsbannersInfo> createState() => _othersshopsbannersInfoState();
}

class _othersshopsbannersInfoState extends State<othersshopsbannersInfo> {
