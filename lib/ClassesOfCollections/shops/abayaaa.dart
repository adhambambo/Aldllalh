...
class AbyatEsdalatShops extends StatelessWidget {
  AbyatEsdalatShops({super.key});
  final Stream<QuerySnapshot> kochiFAcInfo = FirebaseFirestore.instance
      .collection('محلات/محلات/محلات عبايات واسدالات')
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        ...
        title: Text('محلات عبايات واسدالات'),
      ),
      body: Container(
        child: StreamBuilder(
          stream: kochiFAcInfo,
              ...
              itemBuilder: (context, index) {
                ...
                if (factoryData.containsKey('banner')) {
                  return GestureDetector(
                    onTap: () {
                      Get.to(() =>
                          abayashopbannersInfo(factoryDatatwo: factoryData));
                    },
                    child: Image.network(
                  ...
                  return GestureDetector(
                    onTap: () {
                      Get.to(() => abayaShopsinfo(factoryData: factoryData));
                    },
...
//
class abayaShopsinfo extends StatefulWidget {
  final Map<String, dynamic> factoryData;

  const abayaShopsinfo({Key? key, required this.factoryData});

  @override
  State<abayaShopsinfo> createState() => _abayaShopsinfoState();
}

class _abayaShopsinfoState extends State<abayaShopsinfo> {
  final Stream<QuerySnapshot> kochiFAcInfo = FirebaseFirestore.instance
      .collection('محلات/محلات/محلات عبايات واسدالات')
      .snapshots();
...
class abayashopbannersInfo extends StatefulWidget {
  final Map<String, dynamic> factoryDatatwo;

  abayashopbannersInfo({Key? key, required this.factoryDatatwo})
      : super(key: key);

  @override
  State<abayashopbannersInfo> createState() => _abayashopbannersInfoState();
}

class _abayashopbannersInfoState extends State<abayashopbannersInfo> {
