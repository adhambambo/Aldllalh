...
class YouthMenShops extends StatelessWidget {
  YouthMenShops({super.key});
  final Stream<QuerySnapshot> kochiFAcInfo = FirebaseFirestore.instance
      .collection('محلات/محلات/محلات ملابس شبابي ورجالي')
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        ...
        title: Text('محلات ملابس رجالي وشبابي'),
      ),
      body: Container(
        child: StreamBuilder(
          stream: kochiFAcInfo,
              ...
              itemBuilder: (context, index) {
                ...
                if (factoryData.containsKey('banner')) {
                  return GestureDetector(
                    onTap: () {
                      Get.to(() => youyhandmenshopbannersInfo(
                  ...
                  return GestureDetector(
                    onTap: () {
                      Get.to(
                          () => youthandmenshopsinfo(factoryData: factoryData));
                    },
...
//
class youthandmenshopsinfo extends StatefulWidget {
  final Map<String, dynamic> factoryData;

  const youthandmenshopsinfo({Key? key, required this.factoryData});

  @override
  State<youthandmenshopsinfo> createState() => _youthandmenshopsinfoState();
}

class _youthandmenshopsinfoState extends State<youthandmenshopsinfo> {
  final Stream<QuerySnapshot> kochiFAcInfo = FirebaseFirestore.instance
      .collection('محلات/محلات/محلات ملابس شبابي ورجالي')
      .snapshots();
...
class youyhandmenshopbannersInfo extends StatefulWidget {
  final Map<String, dynamic> factoryDatatwo;

  youyhandmenshopbannersInfo({Key? key, required this.factoryDatatwo})
      : super(key: key);

  @override
  State<youyhandmenshopbannersInfo> createState() =>
      _youyhandmenshopbannersInfoState();
}

class _youyhandmenshopbannersInfoState
    extends State<youyhandmenshopbannersInfo> {
