...
class RamdanClothingFactories extends StatefulWidget {
  RamdanClothingFactories({super.key});

  @override
  State<RamdanClothingFactories> createState() =>
      _RamdanClothingFactoriesState();
}

class _RamdanClothingFactoriesState extends State<RamdanClothingFactories> {
  final Stream<QuerySnapshot> tenthOfRamdanClothingFactories = FirebaseFirestore
      .instance
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        ...
        title: Text('مصانع ملابس العاشر من رمضان'),
      ),
      body: Container(
        child: StreamBuilder(
          stream: tenthOfRamdanClothingFactories,
              ...
              itemBuilder: (context, index) {
                ...
                if (factoryData.containsKey('banner')) {
                  return GestureDetector(
                    onTap: () {
                      Get.to(
                          () => ramdanbannersInfo(factoryDatatwo: factoryData));
                    },
                    child: Image.network(
                  ...
                  return GestureDetector(
                    onTap: () {
                      Get.to(() => thramdanfacIanfo(factoryData: factoryData));
                    },
...
class thramdanfacIanfo extends StatefulWidget {
  final Map<String, dynamic> factoryData;

  thramdanfacIanfo({Key? key, required this.factoryData});

  @override
  State<thramdanfacIanfo> createState() => _thramdanfacIanfoState();
}

class _thramdanfacIanfoState extends State<thramdanfacIanfo> {
  final Stream<QuerySnapshot> RamdanInfo = FirebaseFirestore.instance
      .collection('مصانع/مصانع ملابس/مصانع ملابس العاشر من رمضان')
      .snapshots();
...
class ramdanbannersInfo extends StatefulWidget {
  final Map<String, dynamic> factoryDatatwo;

  ramdanbannersInfo({Key? key, required this.factoryDatatwo}) : super(key: key);

  @override
  State<ramdanbannersInfo> createState() => _ramdanbannersInfoState();
}

class _ramdanbannersInfoState extends State<ramdanbannersInfo> {
