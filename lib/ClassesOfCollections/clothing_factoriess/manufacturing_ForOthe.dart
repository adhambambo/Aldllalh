...
class ForOthersClothingFactories extends StatefulWidget {
  const ForOthersClothingFactories({super.key});

  @override
  State<ForOthersClothingFactories> createState() =>
      _ForOthersClothingFactoriesState();
}

class _ForOthersClothingFactoriesState
    extends State<ForOthersClothingFactories> {
  final Stream<QuerySnapshot> manuFactoringForOther = FirebaseFirestore.instance
      .collection('مصانع/مصانع ملابس/مصانع تصنيع للغير')
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        ...
        title: Text('تصنيع للغير'),
      ),
      body: Container(
        child: StreamBuilder(
          stream: manuFactoringForOther,
              ...
              itemBuilder: (context, index) {
                ...
                if (factoryData.containsKey('banner')) {
                  return GestureDetector(
                    onTap: () {
                      Get.to(() =>
                          forotherbannersInfo(factoryDatatwo: factoryData));
                    },
                    child: Image.network(
                  ...
                  return GestureDetector(
                    onTap: () {
                      Get.to(() => forothersInfo(factoryData: factoryData));
                    },
...
//
class forothersInfo extends StatefulWidget {
  final Map<String, dynamic> factoryData;

  const forothersInfo({Key? key, required this.factoryData});

  @override
  State<forothersInfo> createState() => _forothersInfoState();
}

class _forothersInfoState extends State<forothersInfo> {
  final Stream<QuerySnapshot> forothersinfo = FirebaseFirestore.instance
      .collection('مصانع/مصانع ملابس/مصانع تصنيع للغير')
      .snapshots();
...
class forotherbannersInfo extends StatefulWidget {
  final Map<String, dynamic> factoryDatatwo;

  forotherbannersInfo({Key? key, required this.factoryDatatwo})
      : super(key: key);

  @override
  State<forotherbannersInfo> createState() => _forotherbannersInfoState();
}

class _forotherbannersInfoState extends State<forotherbannersInfo> {
