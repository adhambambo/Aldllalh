...
class HomeWearWomenClothingFactories extends StatefulWidget {
  HomeWearWomenClothingFactories({super.key});

  @override
  State<HomeWearWomenClothingFactories> createState() =>
      _HomeWearWomenClothingFactoriesState();
}

class _HomeWearWomenClothingFactoriesState
    extends State<HomeWearWomenClothingFactories> {
  final Stream<QuerySnapshot> usersStream = FirebaseFirestore.instance
      .collection('مصانع/مصانع ملابس/مصانع ملابس هوم ويير حريمي')
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        ...
        title: Text('مصانع ملابس هو وير جريمي'),
      ),
      body: Container(
        child: StreamBuilder(
          stream: usersStream,
              ...
              itemBuilder: (context, index) {
                ...
                if (factoryData.containsKey('banner')) {
                  return GestureDetector(
                    onTap: () {
                      Get.to(() =>
                          homewearbannersInfo(factoryDatatwo: factoryData));
                    },
                    child: Image.network(
                  ...
                  return GestureDetector(
                    onTap: () {
                      Get.to(() => AddingInformayion(factoryData: factoryData));
                    },
...
class AddingInformayion extends StatefulWidget {
  final Map<String, dynamic> factoryData;

  AddingInformayion({Key? key, required this.factoryData});

  @override
  State<AddingInformayion> createState() => _AddingInformayionState();
}

class _AddingInformayionState extends State<AddingInformayion> {
  final Stream<QuerySnapshot> AddInfo = FirebaseFirestore.instance
      .collection('مصانع/مصانع ملابس/مصانع ملابس هوم ويير حريمي')
      .snapshots();
 int _current = 0;
 late int viewsCount;
...
class homewearbannersInfo extends StatefulWidget {
  final Map<String, dynamic> factoryDatatwo;

  homewearbannersInfo({Key? key, required this.factoryDatatwo})
      : super(key: key);

  @override
  State<homewearbannersInfo> createState() => _homewearbannersInfoState();
}

class _homewearbannersInfoState extends State<homewearbannersInfo> {
