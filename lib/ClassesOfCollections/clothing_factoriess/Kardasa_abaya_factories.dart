...
class KerdasaAbayaClothingFactories extends StatefulWidget {
  const KerdasaAbayaClothingFactories({super.key});

  @override
  State<KerdasaAbayaClothingFactories> createState() =>
      _KerdasaAbayaClothingFactoriesState();
}

class _KerdasaAbayaClothingFactoriesState
    extends State<KerdasaAbayaClothingFactories> {
  final Stream<QuerySnapshot> AbayatKerdasa = FirebaseFirestore.instance
      .collection('مصانع/مصانع ملابس/مصانع عبايات كرداسة')
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        ...
        title: Text('مصانع عبايات كرداسة'),
      ),
      body: Container(
        child: StreamBuilder(
              ...
              itemBuilder: (context, index) {
                ...
                if (factoryData.containsKey('banner')) {
                  return GestureDetector(
                    onTap: () {
                      Get.to(() =>
                          kerdaasabannersInfo(factoryDatatwo: factoryData));
                    },
                    child: Image.network(
                  ...
                  return GestureDetector(
                    onTap: () {
                      Get.to(() => KerdasaABAYAfac(factoryData: factoryData));
                    },
...
//
class KerdasaABAYAfac extends StatefulWidget {
  final Map<String, dynamic> factoryData;

  const KerdasaABAYAfac({Key? key, required this.factoryData});

  @override
  State<KerdasaABAYAfac> createState() => _KerdasaABAYAfacState();
}

class _KerdasaABAYAfacState extends State<KerdasaABAYAfac> {
...
class kerdaasabannersInfo extends StatefulWidget {
  final Map<String, dynamic> factoryDatatwo;

  kerdaasabannersInfo({Key? key, required this.factoryDatatwo})
      : super(key: key);

  @override
  State<kerdaasabannersInfo> createState() => _kerdaasabannersInfoState();
}

class _kerdaasabannersInfoState extends State<kerdaasabannersInfo> {
