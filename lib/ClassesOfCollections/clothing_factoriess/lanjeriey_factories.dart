...
class LinjeriyClothingFactories extends StatefulWidget {
  const LinjeriyClothingFactories({super.key});

  @override
  State<LinjeriyClothingFactories> createState() =>
      _LinjeriyClothingFactoriesState();
}

class _LinjeriyClothingFactoriesState extends State<LinjeriyClothingFactories> {
  final Stream<QuerySnapshot> linjeriyFactories = FirebaseFirestore.instance
      .collection('مصانع/مصانع ملابس/مصانع داخلي ولانجيري')
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        ...
        title: Text('مصانع داخلي ولانجيري'),
      ),
      body: Container(
        child: StreamBuilder(
          stream: linjeriyFactories,
              ...
              itemBuilder: (context, index) {
                ...
                if (factoryData.containsKey('banner')) {
                  return GestureDetector(
                    onTap: () {
                      Get.to(() =>
                          linjeriebannersInfo(factoryDatatwo: factoryData));
                    },
                    child: Image.network(
                  ...
                  return GestureDetector(
                    onTap: () {
                      Get.to(() => linjerieyfacinfo(factoryData: factoryData));
                    },
...
//
class linjerieyfacinfo extends StatefulWidget {
  final Map<String, dynamic> factoryData;

  const linjerieyfacinfo({Key? key, required this.factoryData});

  @override
  State<linjerieyfacinfo> createState() => _linjerieyfacinfoState();
}

class _linjerieyfacinfoState extends State<linjerieyfacinfo> {
  final Stream<QuerySnapshot> linjerieinfo = FirebaseFirestore.instance
      .collection('مصانع/مصانع ملابس/مصانع داخلي ولانجيري')
      .snapshots();
...
class linjeriebannersInfo extends StatefulWidget {
  final Map<String, dynamic> factoryDatatwo;

  linjeriebannersInfo({Key? key, required this.factoryDatatwo})
      : super(key: key);

  @override
  State<linjeriebannersInfo> createState() => _linjeriebannersInfoState();
}

class _linjeriebannersInfoState extends State<linjeriebannersInfo> {
