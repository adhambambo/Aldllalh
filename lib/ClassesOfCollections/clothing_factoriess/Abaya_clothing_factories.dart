...
class AbayatEsdalatClothingFactories extends StatelessWidget {
  AbayatEsdalatClothingFactories({super.key});
  final Stream<QuerySnapshot> AbayatEsdalat = FirebaseFirestore.instance
      .collection('مصانع/مصانع ملابس/مصانع ملابس عبايات واسدالات')
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        ...
        title: Text('مصانع ملابس عبايات واسدالات'),
      ),
      body: Container(
        child: StreamBuilder(
          stream: AbayatEsdalat,
              ...
              itemBuilder: (context, index) {
                ...
                if (factoryData.containsKey('banner')) {
                  return GestureDetector(
                    onTap: () {
                      Get.to(
                          () => abayabannersInfo(factoryDatatwo: factoryData));
                    },
                    child: Image.network(
                  ...
                  return GestureDetector(
                    onTap: () {
                      Get.to(() => abayarEsdalatInfo(factoryData: factoryData));
                    },
...
//
class abayarEsdalatInfo extends StatefulWidget {
  final Map<String, dynamic> factoryData;

  abayarEsdalatInfo({Key? key, required this.factoryData});

  @override
  State<abayarEsdalatInfo> createState() => _abayarEsdalatInfoState();
}

class _abayarEsdalatInfoState extends State<abayarEsdalatInfo> {
  final Stream<QuerySnapshot> abayatinfo = FirebaseFirestore.instance
      .collection('مصانع/مصانع ملابس/مصانع ملابس عبايات واسدالات')
      .snapshots();
    int _current = 0;
    late int viewsCount;
...
class abayabannersInfo extends StatefulWidget {
  final Map<String, dynamic> factoryDatatwo;

  abayabannersInfo({Key? key, required this.factoryDatatwo}) : super(key: key);

  @override
  State<abayabannersInfo> createState() => _abayabannersInfoState();
}

class _abayabannersInfoState extends State<abayabannersInfo> {
