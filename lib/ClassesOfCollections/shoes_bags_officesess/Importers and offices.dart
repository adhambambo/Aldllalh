...
class importersShoesBagsOffices extends StatelessWidget {
  importersShoesBagsOffices({super.key});
  final Stream<QuerySnapshot> kochiFAcInfo = FirebaseFirestore.instance
      .collection('مكاتب/مكاتب احذية وشنط/مستوردين ومكاتب احذية وشنط')
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        ...
        title: Text('مستوردين ومكاتب احذية وشنط'),
      ),
      body: Container(
        child: StreamBuilder(
          stream: kochiFAcInfo,
              ...
              itemBuilder: (context, index) {
                ...
                if (factoryData.containsKey('banner')) {
                  return GestureDetector(
                    onTap: () {
                      Get.to(() =>
                          importerssssbannersInfo(factoryDatatwo: factoryData));
                    },
                    child: Image.network(
                  ...
                  return GestureDetector(
                    onTap: () {
                      Get.to(
                          () => importersofficesinfo(factoryData: factoryData));
                    },
...
//
class importersofficesinfo extends StatefulWidget {
  final Map<String, dynamic> factoryData;

  const importersofficesinfo({Key? key, required this.factoryData});

  @override
  State<importersofficesinfo> createState() => _importersofficesinfoState();
}

class _importersofficesinfoState extends State<importersofficesinfo> {
  final Stream<QuerySnapshot> kochiFAcInfo = FirebaseFirestore.instance
      .collection('مكاتب/مكاتب احذية وشنط/مستوردين ومكاتب احذية وشنط')
      .snapshots();
   int _current = 0;
...
class importerssssbannersInfo extends StatefulWidget {
  final Map<String, dynamic> factoryDatatwo;

  importerssssbannersInfo({Key? key, required this.factoryDatatwo})
      : super(key: key);

  @override
  State<importerssssbannersInfo> createState() =>
      _importerssssbannersInfoState();
}

class _importerssssbannersInfoState extends State<importerssssbannersInfo> {
