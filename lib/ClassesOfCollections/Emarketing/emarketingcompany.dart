...
class EmarketingCompany extends StatelessWidget {
  EmarketingCompany({super.key});
  final Stream<QuerySnapshot> kochiFAcInfo = FirebaseFirestore.instance
      .collection('كولكشن/تسويق الكتروني/شركات تسويق الكتروني')
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        ...
        title: Text('شركات تسويق الكتروني'),
      ),
      body: Container(
        child: StreamBuilder(
          stream: kochiFAcInfo,
              ...
              itemBuilder: (context, index) {
                ...
                if (factoryData.containsKey('banner')) {
                  return GestureDetector(
                    onTap: () {
                      Get.to(() =>
                          emarketingbannersInfo(factoryDatatwo: factoryData));
                    },
                    child: Image.network(
                  ...
                  return GestureDetector(
                    onTap: () {
                      Get.to(() =>
                          EmarketingCompanyinfo(factoryData: factoryData));
                    },
...
//
class EmarketingCompanyinfo extends StatefulWidget {
  final Map<String, dynamic> factoryData;

  const EmarketingCompanyinfo({Key? key, required this.factoryData});

  @override
  State<EmarketingCompanyinfo> createState() => _EmarketingCompanyinfoState();
}

class _EmarketingCompanyinfoState extends State<EmarketingCompanyinfo> {
  final Stream<QuerySnapshot> kochiFAcInfo = FirebaseFirestore.instance
      .collection('كولكشن/تسويق الكتروني/شركات تسويق الكتروني')
      .snapshots();
...
class emarketingbannersInfo extends StatefulWidget {
  final Map<String, dynamic> factoryDatatwo;

  emarketingbannersInfo({Key? key, required this.factoryDatatwo})
      : super(key: key);

  @override
  State<emarketingbannersInfo> createState() => _emarketingbannersInfoState();
}

class _emarketingbannersInfoState extends State<emarketingbannersInfo> {
