...
class Affiliates extends StatelessWidget {
  Affiliates({super.key});
  final Stream<QuerySnapshot> kochiFAcInfo = FirebaseFirestore.instance
      .collection('كولكشن/تسويق الكتروني/مسوقات بالعمولة')
              ...
              itemBuilder: (context, index) {
                ...
                if (factoryData.containsKey('banner')) {
                  return GestureDetector(
                    onTap: () {
                      Get.to(() =>
                          afiiliatebannersInfo(factoryDatatwo: factoryData));
                    },
                    child: Image.network(
                  ...
                  return GestureDetector(
                    onTap: () {
                      Get.to(() => Affilatesinfo(factoryData: factoryData));
                    },
...
//
class Affilatesinfo extends StatefulWidget {
  final Map<String, dynamic> factoryData;

  const Affilatesinfo({Key? key, required this.factoryData});

  @override
  State<Affilatesinfo> createState() => _AffilatesinfoState();
}

class _AffilatesinfoState extends State<Affilatesinfo> {
  final Stream<QuerySnapshot> kochiFAcInfo = FirebaseFirestore.instance
      .collection('كولكشن/تسويق الكتروني/مسوقات بالعمولة')
      .snapshots();
...
class afiiliatebannersInfo extends StatefulWidget {
  final Map<String, dynamic> factoryDatatwo;

  afiiliatebannersInfo({Key? key, required this.factoryDatatwo})
      : super(key: key);

  @override
  State<afiiliatebannersInfo> createState() => _afiiliatebannersInfoState();
}

class _afiiliatebannersInfoState extends State<afiiliatebannersInfo> {
