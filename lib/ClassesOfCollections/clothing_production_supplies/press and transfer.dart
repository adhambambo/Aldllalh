...
class pressTransfeClothingProductionSupplies extends StatelessWidget {
  pressTransfeClothingProductionSupplies({super.key});
  final Stream<QuerySnapshot> kochiFAcInfo = FirebaseFirestore.instance
      .collection('مصانع/مستلزمات انتاج ملابس/كبس استرس وترانسفير')
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        ...
        title: Text('كبس استرس وترانسفير'),
      ),
      body: Container(
        child: StreamBuilder(
          stream: kochiFAcInfo,
              ...
              itemBuilder: (context, index) {
                ...
                if (factoryData.containsKey('banner')) {
                  return GestureDetector(
                    onTap: () {
                      Get.to(() => pressandtransferbannersInfo(
                  ...
                  return GestureDetector(
                    onTap: () {
                      Get.to(() => pressTransfergclothingeiinfo(
...
//
class pressTransfergclothingeiinfo extends StatefulWidget {
  final Map<String, dynamic> factoryData;

  const pressTransfergclothingeiinfo({Key? key, required this.factoryData});

  @override
  State<pressTransfergclothingeiinfo> createState() =>
      _pressTransfergclothingeiinfoState();
}

class _pressTransfergclothingeiinfoState
    extends State<pressTransfergclothingeiinfo> {
  final Stream<QuerySnapshot> kochiFAcInfo = FirebaseFirestore.instance
      .collection('مصانع/مستلزمات انتاج ملابس/كبس استرس وترانسفير')
      .snapshots();
...
class pressandtransferbannersInfo extends StatefulWidget {
  final Map<String, dynamic> factoryDatatwo;

  pressandtransferbannersInfo({Key? key, required this.factoryDatatwo})
      : super(key: key);

  @override
  State<pressandtransferbannersInfo> createState() =>
      _pressandtransferbannersInfoState();
}

class _pressandtransferbannersInfoState
    extends State<pressandtransferbannersInfo> {
