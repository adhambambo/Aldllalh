...
class otherClothingProductionSupplies extends StatelessWidget {
  otherClothingProductionSupplies({super.key});
  final Stream<QuerySnapshot> kochiFAcInfo = FirebaseFirestore.instance
      .collection('مصانع/مستلزمات انتاج ملابس/مستلزمات انتاج ملابس اخري')
              ...
              itemBuilder: (context, index) {
                ...
                if (factoryData.containsKey('banner')) {
                  return GestureDetector(
                    onTap: () {
                      Get.to(() =>
                          othersclothbannersInfo(factoryDatatwo: factoryData));
                    },
                    child: Image.network(
                  ...
                  return GestureDetector(
                    onTap: () {
                      Get.to(
                          () => otherclothingeiinfo(factoryData: factoryData));
                    },
...
//
class otherclothingeiinfo extends StatefulWidget {
  final Map<String, dynamic> factoryData;

  const otherclothingeiinfo({Key? key, required this.factoryData});

  @override
  State<otherclothingeiinfo> createState() => _otherclothingeiinfoState();
}

class _otherclothingeiinfoState extends State<otherclothingeiinfo> {
  final Stream<QuerySnapshot> kochiFAcInfo = FirebaseFirestore.instance
      .collection('مصانع/مستلزمات انتاج ملابس/مستلزمات انتاج ملابس اخري')
      .snapshots();
...
class othersclothbannersInfo extends StatefulWidget {
  final Map<String, dynamic> factoryDatatwo;

  othersclothbannersInfo({Key? key, required this.factoryDatatwo})
      : super(key: key);

  @override
  State<othersclothbannersInfo> createState() => _othersclothbannersInfoState();
}

class _othersclothbannersInfoState extends State<othersclothbannersInfo> {
