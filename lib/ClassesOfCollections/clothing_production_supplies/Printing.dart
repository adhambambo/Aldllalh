...
class printingClothingProductionSupplies extends StatelessWidget {
  printingClothingProductionSupplies({super.key});
  final Stream<QuerySnapshot> kochiFAcInfo = FirebaseFirestore.instance
      .collection('مصانع/مستلزمات انتاج ملابس/مطابع')
              ...
              itemBuilder: (context, index) {
                ...
                if (factoryData.containsKey('banner')) {
                  return GestureDetector(
                    onTap: () {
                      Get.to(() =>
                          printingbannersInfo(factoryDatatwo: factoryData));
                    },
                    child: Image.network(
                  ...
                  return GestureDetector(
                    onTap: () {
                      Get.to(() =>
                          printingclothingeiinfo(factoryData: factoryData));
                    },
...
//
class printingclothingeiinfo extends StatefulWidget {
  final Map<String, dynamic> factoryData;

  const printingclothingeiinfo({Key? key, required this.factoryData});

  @override
  State<printingclothingeiinfo> createState() => _printingclothingeiinfoState();
}

class _printingclothingeiinfoState extends State<printingclothingeiinfo> {
  final Stream<QuerySnapshot> kochiFAcInfo = FirebaseFirestore.instance
      .collection('مصانع/مستلزمات انتاج ملابس/مطابع')
      .snapshots();
...
class printingbannersInfo extends StatefulWidget {
  final Map<String, dynamic> factoryDatatwo;

  printingbannersInfo({Key? key, required this.factoryDatatwo})
      : super(key: key);

  @override
  State<printingbannersInfo> createState() => _printingbannersInfoState();
}

class _printingbannersInfoState extends State<printingbannersInfo> {
