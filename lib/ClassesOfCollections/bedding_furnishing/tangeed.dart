...
class TangeedsBeddingFurnishing extends StatelessWidget {
  TangeedsBeddingFurnishing({super.key});
  final Stream<QuerySnapshot> kochiFAcInfo = FirebaseFirestore.instance
      .collection('مصانع/مفروشات ووبريات/اقمشة تنجيد')
              ...
              itemBuilder: (context, index) {
                ...
                if (factoryData.containsKey('banner')) {
                  return GestureDetector(
                    onTap: () {
                      Get.to(() =>
                          tangeedbannersInfo(factoryDatatwo: factoryData));
                    },
                    child: Image.network(
                  ...
                  return GestureDetector(
                    onTap: () {
                      Get.to(
                          () => Tangeedlothingeiinfo(factoryData: factoryData));
                    },
...
//
class Tangeedlothingeiinfo extends StatefulWidget {
  final Map<String, dynamic> factoryData;

  Tangeedlothingeiinfo({Key? key, required this.factoryData});

  @override
  State<Tangeedlothingeiinfo> createState() => _TangeedlothingeiinfoState();
}

class _TangeedlothingeiinfoState extends State<Tangeedlothingeiinfo> {
  final Stream<QuerySnapshot> kochiFAcInfo = FirebaseFirestore.instance
      .collection('مصانع/مفروشات ووبريات/اقمشة تنجيد')
      .snapshots();
...
class tangeedbannersInfo extends StatefulWidget {
  final Map<String, dynamic> factoryDatatwo;

  tangeedbannersInfo({Key? key, required this.factoryDatatwo})
      : super(key: key);

  @override
  State<tangeedbannersInfo> createState() => _tangeedbannersInfoState();
}

class _tangeedbannersInfoState extends State<tangeedbannersInfo> {
