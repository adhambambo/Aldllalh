...
class TowesSheetssBeddingFurnishing extends StatelessWidget {
  TowesSheetssBeddingFurnishing({super.key});
  final Stream<QuerySnapshot> kochiFAcInfo = FirebaseFirestore.instance
      .collection('مصانع/مفروشات ووبريات/فوط وملايات سرير والحفه')
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        ...
        title: Text('فوط وملايات سرير ولحفة'),
      ),
      body: Container(
        child: StreamBuilder(
          stream: kochiFAcInfo,
              ...
              itemBuilder: (context, index) {
                ...
                if (factoryData.containsKey('banner')) {
                  return GestureDetector(
                    onTap: () {
                      Get.to(() =>
                          towelsbedbannersInfo(factoryDatatwo: factoryData));
                    },
                    child: Image.network(
                  ...
                  return GestureDetector(
                    onTap: () {
                      Get.to(() => TowelsBedSheetslothingeiinfo(
...
//
class TowelsBedSheetslothingeiinfo extends StatefulWidget {
  final Map<String, dynamic> factoryData;

  TowelsBedSheetslothingeiinfo({Key? key, required this.factoryData});

  @override
  State<TowelsBedSheetslothingeiinfo> createState() =>
      _TowelsBedSheetslothingeiinfoState();
}

class _TowelsBedSheetslothingeiinfoState
    extends State<TowelsBedSheetslothingeiinfo> {
  final Stream<QuerySnapshot> kochiFAcInfo = FirebaseFirestore.instance
      .collection('مصانع/مفروشات ووبريات/فوط وملايات سرير والحفه')
      .snapshots();
 int _current = 0;
 late int viewsCount;
  ...
  void initState() {
    ...
    viewsCount = widget.factoryData['views'] ?? 185;
    _incrementViews();
  }
...
class towelsbedbannersInfo extends StatefulWidget {
  final Map<String, dynamic> factoryDatatwo;

  towelsbedbannersInfo({Key? key, required this.factoryDatatwo})
      : super(key: key);

  @override
  State<towelsbedbannersInfo> createState() => _towelsbedbannersInfoState();
}

class _towelsbedbannersInfoState extends State<towelsbedbannersInfo> {
