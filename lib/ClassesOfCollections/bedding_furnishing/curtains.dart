...
class CurtainsBeddingFurnishing extends StatelessWidget {
  CurtainsBeddingFurnishing({super.key});
  final Stream<QuerySnapshot> kochiFAcInfo = FirebaseFirestore.instance
      .collection('مصانع/مفروشات ووبريات/ستائر')
              ...
              itemBuilder: (context, index) {
                ...
                if (factoryData.containsKey('banner')) {
                  return GestureDetector(
                    onTap: () {
                      Get.to(() =>
                          curtainsbannersInfo(factoryDatatwo: factoryData));
                    },
                    child: Image.network(
                  ...
                  return GestureDetector(
                    onTap: () {
                      Get.to(() =>
                          Curtainslothingeiinfo(factoryData: factoryData));
                    },
...
//const
class Curtainslothingeiinfo extends StatefulWidget {
  final Map<String, dynamic> factoryData;

  Curtainslothingeiinfo({Key? key, required this.factoryData});

  @override
  State<Curtainslothingeiinfo> createState() => _CurtainslothingeiinfoState();
}

class _CurtainslothingeiinfoState extends State<Curtainslothingeiinfo> {
  final Stream<QuerySnapshot> kochiFAcInfo = FirebaseFirestore.instance
      .collection('مصانع/مفروشات ووبريات/ستائر')
      .snapshots();
  ...
  void initState() {
    ...
    viewsCount = widget.factoryData['views'] ?? 188;
    _incrementViews();
  }
...
class curtainsbannersInfo extends StatefulWidget {
  final Map<String, dynamic> factoryDatatwo;

  curtainsbannersInfo({Key? key, required this.factoryDatatwo})
      : super(key: key);

  @override
  State<curtainsbannersInfo> createState() => _curtainsbannersInfoState();
}

class _curtainsbannersInfoState extends State<curtainsbannersInfo> {
