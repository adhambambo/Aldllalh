...
class cairoFashion extends StatefulWidget {
  const cairoFashion({super.key});

  @override
  State<cairoFashion> createState() => _cairoFashionState();
}

class _cairoFashionState extends State<cairoFashion> {
  final Stream<QuerySnapshot> AbayatKerdasa = FirebaseFirestore.instance
      .collection('populars/exhibtions/cairofashion')
              ...
              itemBuilder: (context, index) {
                ...
                if (factoryData.containsKey('banner')) {
                  return GestureDetector(
                    onTap: () {
                      Get.to(() =>
                          cairoFashionbannersInfo(factoryDatatwo: factoryData));
                    },
                    child: Image.network(
                  ...
                  return GestureDetector(
                    onTap: () {
                      Get.to(() => cairoFashioninfo(factoryData: factoryData));
                    },
...
//
class cairoFashioninfo extends StatefulWidget {
  final Map<String, dynamic> factoryData;

  const cairoFashioninfo({Key? key, required this.factoryData});

  @override
  State<cairoFashioninfo> createState() => _cairoFashioninfoState();
}

class _cairoFashioninfoState extends State<cairoFashioninfo> {
...
int _current = 0;
  ...
  Widget build(BuildContext context) {
    final List<String> imgList = [
      if (widget.factoryData['image1']?.isNotEmpty == true)
        widget.factoryData['image1']!,
      if (widget.factoryData['image2']?.isNotEmpty == true)
        widget.factoryData['image2']!,
      if (widget.factoryData['image3']?.isNotEmpty == true)
        widget.factoryData['image3']!,
      if (widget.factoryData['image4']?.isNotEmpty == true)
        widget.factoryData['image4']!,
      if (widget.factoryData['image5']?.isNotEmpty == true)
        widget.factoryData['image5']!,
      if (widget.factoryData['image6']?.isNotEmpty == true)
        widget.factoryData['image6']!,
      if (widget.factoryData['image7']?.isNotEmpty == true)
        widget.factoryData['image7']!,
      if (widget.factoryData['image8']?.isNotEmpty == true)
        widget.factoryData['image8']!,
      if (widget.factoryData['image9']?.isNotEmpty == true)
        widget.factoryData['image9']!,
      if (widget.factoryData['image10']?.isNotEmpty == true)
        widget.factoryData['image10']!,
    ];
...
class cairoFashionbannersInfo extends StatefulWidget {
  final Map<String, dynamic> factoryDatatwo;

  cairoFashionbannersInfo({Key? key, required this.factoryDatatwo})
      : super(key: key);

  @override
  State<cairoFashionbannersInfo> createState() => _cairoFashionbannersInfoState();
}

class _cairoFashionbannersInfoState extends State<cairoFashionbannersInfo> {
